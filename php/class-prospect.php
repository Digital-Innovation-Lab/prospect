			...
			foreach ($query->posts as $rec) {
				...
				case 'list':
					...
					if ($a['content_attr'] != null && isset($the_rec->att_data[$a['content_attr']])) {
						$html .= '<p class="prospect-list-content">'.Prospect::get_att_val($assoc_atts, $a['content_attr'], $the_rec->att_data).'</p>';
					}
					$html .= '</div>';
					break;
				case 'cards':
					...
					if ($a['content_attr'] != null && isset($the_rec->att_data[$a['content_attr']])) {
						$html .= '<br/><span class="content">'.Prospect::get_att_val($assoc_atts, $a['content_attr'], $the_rec->att_data).'</span>';
					}
					$html .= '</p></div>';
					break;
				case 'images':
