...
<!-- VueJS Template Dialog Component -- DONE -->
...
<script id="dialog-message" type='text/x-template'>
	<dialog title=<?php _e('"Display Hint"', 'prospect'); ?> size="size">
		{{message}}
	</dialog>
</script>
...
<script id="dialog-confirm" type='text/x-template'>
	<dialog title=<?php _e('"Confirm"', 'prospect'); ?> cancel="true" size="size" v-on:save="emit('ok')">
		{{message}}
	</dialog>
</script>
...
<script id="dialog-legend-text" type='text/x-template'>
	<dialog title=<?php _e('"Text Legend Entry"', 'prospect'); ?> cancel="true" v-on:save="emit('save')">
...
<script id="dialog-legend-number" type='text/x-template'>
	<dialog title=<?php _e('"Number Legend Entry"', 'prospect'); ?> cancel="true" v-on:save="emit('save')">
...
<script id="dialog-legend-dates" type='text/x-template'>
	<dialog title=<?php _e('"Date Legend Entry"', 'prospect'); ?> cancel="true" v-on:save="emit('save')">
...
<script id="dialog-move-vocab-lone" type='text/x-template'>
	<dialog title=<?php _e('"Move Vocabulary Entry"', 'prospect'); ?> cancel="true" v-on:save="emit('save')">
		<?php _e('Move this Term so it becomes a child of', 'prospect'); ?> <br/>
		<select v-model='newParent'>
			<option v-for="parent in parents" v-bind:value="parent">
    			{{ parent }}
			</option>
		</select>
		<br/>
...
<script id="dialog-move-vocab-parent" type='text/x-template'>
	<dialog title=<?php _e('"Move Vocabulary Children"', 'prospect'); ?> cancel="true" v-on:save="emit('save')">
...
<script id="dialog-reset-colors" type='text/x-template'>
	<dialog title=<?php _e('"Reset Colors"', 'prospect'); ?> cancel="true" v-on:save="emit('save')">
...
<script id="dialog-choose-color-clear" type='text/x-template'>
	<dialog title=<?php _e('"Choose Color"', 'prospect'); ?> cancel="true" v-on:save="emit('save')">
...
<script id="dialog-copy-legend" type='text/x-template'>
	<dialog title=<?php _e('"Choose Attribute"', 'prospect'); ?> cancel="true" v-on:save="emit('save')">
		...
		<select id="choose-att" v-model='fid'>
			<option v-for="other in others" v-bind:value="other.id">
    			{{other.l}} ({{other.id}})
			</option>
		</select>
	</dialog>
...
<script id="component-edit-vocab" type='text/x-template'>
	...
	<button v-on:click="addTerms"><?php _e('Collect Terms', 'prospect'); ?></button>
	...
	<div class="legend-vocab">
		<ol class="vocab-parent">
			<li v-for="(lgnd1, index1) in theLegend" class="legend-data-entry">
...
<!-- Edit settings for Text Attribute -->
<script id="component-edit-text" type='text/x-template'>
...
<script id="vue-outer" type='text/x-template'>
	...
	<select v-model='chosenCF'>
		<option v-for="cf in cfs" v-bind:value="cf">
			{{cf}})
		</option>
	</select>
