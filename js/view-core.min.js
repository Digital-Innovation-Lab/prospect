...
function(c,d){g=c.o;a.lOps.push(100*g);var e;e=PMapHub.createMapLayer(c.lid,g,a.lMap,null);a.mapLayers.push(e)});var h=_.template(document.getElementById("dltext-v-map").innerHTML);jQuery("#view-frame-"+f+" div.view-controls").append(h({vi:f}));jQuery("#map-zoom-"+f).button({text:!1,icons:{primary:"ui-icon-plus"}}).click(function(){a.lMap.zoomIn()});jQuery("#map-unzoom-"+f).button({text:!1,icons:{primary:"ui-icon-minus"}}).click(function(){a.lMap.zoomOut()});jQuery("#map-reset-"+f).button({text:!1,
...
d){var k=c.lOps[d];h=jQuery('<div class="op-layer" data-i="'+d+'">'+c.mapLayers[d].options.layerName+' <input type=range class="op-slider" min=0 max=100 value='+k+" step=5></div>");h.find(".op-slider").on("change",function(){e[d]=jQuery(this).val();c.mapLayers[d].setOpacity(e[d]/100)});e.push(k);g.append(h)});var l=jQuery("#dialog-opacities").dialog({height:300,width:500,modal:!0,buttons:[{text:dlText.ok,click:function(){f=!1;l.dialog("close");c.bOp=d;e.forEach(function(a,d){c.lOps[d]=e[d]})}},{text:dlText.cancel,
...
this.settings.lyrs.forEach(function(a,d){var k=c.lOps[d];h=jQuery('<div class="op-layer" data-i="'+d+'">'+a.gid+' <input type=range class="op-slider" min=0 max=100 value='+k+" step=5></div>");h.find(".op-slider").on("change",function(){var a=jQuery(this).val();e[d]=a;a/=100;c.mapLayers[d].eachLayer(function(k){k.setOpacity(a)})});e.push(k);g.append(h)});var l=jQuery("#dialog-opacities").dialog({height:300,width:500,modal:!0,buttons:[{text:dlText.ok,click:function(){f=!1;l.dialog("close");c.bOp=d;
...
function(a,k){var d=c.toggleSel(a.ai);d3.select(this).classed("obj-sel",d)});if(p){p.sort(function(a,k){return PData.strcmp(k.id,a.id)});var D=[];p.forEach(function(a){a.p&&a.p.forEach(function(k){e=_.sortedIndex(p,{id:k},"id");if(e<p.length){var c=p[e];c.id===k&&D.push({f:a.c,t:c.c,c:a.l})}})});this.gRecs.selectAll(".recline").data(D).enter().append("line").attr("class","recline").attr("x1",function(a){return c.xScale(a.f[0])}).attr("y1",function(a){return c.yScale(a.f[1])}).attr("x2",function(a){return c.xScale(a.t[0])}).attr("y2",
...
d3.select("#base-"+d).attr("opacity",e/100)});h.append(l);this.lOps.forEach(function(a,k){l=jQuery('<div class="op-layer" data-i="'+k+'">Overlay '+(k+1)+' <input type=range class="op-slider" min=0 max=100 value='+a+" step=5></div>");l.find(".op-slider").on("change",function(){f[k]=jQuery(this).val();d3.select("#ol-"+d+"-"+k).attr("opacity",f[k]/100)});f.push(a);h.append(l)});var m=jQuery("#dialog-opacities").dialog({height:300,width:320,modal:!0,buttons:[{text:dlText.ok,click:function(){g=!1;m.dialog("close");
