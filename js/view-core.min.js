...
d.c).classed("thick",!0);this.parentElement.appendChild(this);break}}else d3.select(this).attr("stroke","black").classed("thick",!1)});l.select("text").attr("fill",function(a){return a.linked?"white":"black"});d3.select(this).attr("fill","yellow");PState.set(PSTATE_READY)}var e=this,f=[],h,g,l;a?this.stream=a:a=this.stream;this.svg.selectAll(".node").remove();this.svg.selectAll(".link").remove();0<this.recSel.length&&(this.recSel=[]);this.preRender();if(0===a.l)this.svg.attr("width","10").attr("height",
"10");else{var n={children:[]};(function(){var c,k=0,d=0,f,h,p,g=[],l=null,q,t;c=a.t[0];t=e.vFrame.getSelLegend(0);q=PData.aByID(t);l=e.vFrame.getSelFeatAtts(0);0<l.length&&0<c.n&&(e.tUsed[0]=!0);a:for(;d<a.l;){for(;0===c.n||c.i+c.n===d||0===l.length;){0<g.length&&(n.children.push({ti:k,children:g}),g=[]);if(++k===PData.eTNum())break a;g=[];c=a.t[k];t=e.vFrame.getSelLegend(k);q=PData.aByID(t);l=e.vFrame.getSelFeatAtts(k);0<l.length&&0<c.n&&(e.tUsed[k]=!0)}p=a.s[d];f=PData.rByN(p);h=f.a[t];"undefined"!==
typeof h&&(fData=PData.lRecs(h,q,l,!1))&&(e.rMap[p>>4]|=1<<(p&15),g.push({r:f,ai:p,c:fData.v,l:!1,children:[]}));d++}0<g.length&&n.children.push({ti:k,children:g})})();n.children.forEach(function(a){var c=e.settings.pAtts[a.ti],k;a.children.forEach(function(a){c.forEach(function(c){k=a.r.a[c.pid];"undefined"!==typeof k&&k.forEach(function(k){var d=!1,m,e=0;a:for(;e<n.children.length;e++)for(var w=n.children[e],u=0;u<w.children.length;u++)if(m=w.children[u],m.r.id==k){d=!0;break a}d&&(a.l=!0,m.l=!0,
f.push({source:a,target:m,c:c.clr}))})})})});this.prune&&n.children.forEach(function(a){for(var c=a.children.length-1;0<=c;c--)a.children[c].l||a.children.splice(c,1)});var t=0;n.children.forEach(function(a){t+=a.children.length});this.inc=360/(t+n.children.length);if(0===t)this.svg.attr("width","10").attr("height","10");else{g=Math.max((14*t+20)/(2*Math.PI),30);var k=this.settings.lw;"string"===typeof k&&(k=parseInt(k));this.cr=k+12+g;k=2*this.cr;this.svg.attr("width",k).attr("height",k);this.center.attr("transform",
"translate("+this.cr+","+this.cr+")");g=d3.layout.cluster().size([360,g]).sort(null).nodes(n);l=this.center.append("g").selectAll(".node").data(g.filter(function(a){return!a.children})).enter().append("g").attr("class","node").attr("transform",function(a){return"rotate("+(a.x-90)+")translate("+(a.y+8)+",0)"});l.append("circle").attr("r","5").style("fill",function(a){return a.c}).on("click",c);l.append("text").attr("dy",".31em").attr("dx",function(a){return 180>a.x?"10":"-10"}).attr("transform",function(a){return 180>
a.x?"":"rotate(180)"}).style("text-anchor",function(a){return 180>a.x?"start":"end"}).attr("fill","black").text(function(a){return a.r.l}).on("click",d);g=d3.layout.bundle();k=d3.svg.line.radial().interpolate("bundle").tension(.85).radius(function(a){return a.y}).angle(function(a){return a.x/180*Math.PI});h=this.center.append("g").selectAll(".link").data(g(f)).enter().append("path").each(function(a){a.s=a[0];a.t=a[a.length-1]}).attr("class","link").attr("d",k).attr("stroke","black")}}};
...
f.append("g");a.brushg.attr("class","brush").call(a.brush);a.brushg.selectAll("rect").attr("y",-1).attr("height",d+4);a.brushg.selectAll(".resize").append("path").attr("d",function(a){var c=(a=+("e"==a))?1:-1,m=d/4;return"M"+.5*c+","+m+"A6,6 0 0 "+a+" "+6.5*c+","+(m+6)+"V"+(2*m-6)+"A6,6 0 0 "+a+" "+.5*c+","+2*m+"ZM"+2.5*c+","+(m+8)+"V"+(2*m-8)+"M"+4.5*c+","+(m+8)+"V"+(2*m-8)})}else this.min="undefined"===typeof att.r.min?0:att.r.min,this.max="undefined"===typeof att.r.max?100:att.r.max;this.refreshBoxes();
