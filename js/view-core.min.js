Array.prototype.findIndex||(Array.prototype.findIndex=function(a,c){if(null==this)throw new TypeError("Array.prototype.find called on null or undefined");if("function"!==typeof a)throw new TypeError("predicate must be a function");for(var d=Object(this),e=d.length>>>0,f,h=0;h<e;h++)if(f=d[h],a.call(c,f,h,d))return h;return-1});
...
VizMap.prototype.doOptions=function(){function a(){f&&(c.baseMap.setOpacity(c.bOp/100),c.lOps.forEach(function(a,e){c.mapLayers[e].setOpacity(a/100)}));h.empty()}var c=this,d=this.bOp,e=[],f=!0,h=jQuery("#dialog-opacities div.layer-list"),g=jQuery('<div class="op-layer" data-i="-1">Base Map <input type=range class="op-slider" min=0 max=100 value='+this.bOp+" step=5></div>");g.find(".op-slider").on("change",function(){d=jQuery(this).val();c.baseMap.setOpacity(d/100)});h.append(g);this.settings.lyrs.forEach(function(a,
d){var k=c.lOps[d];g=jQuery('<div class="op-layer" data-i="'+d+'">'+c.mapLayers[d].options.layerName+' <input type=range class="op-slider" min=0 max=100 value='+k+" step=5></div>");g.find(".op-slider").on("change",function(){e[d]=jQuery(this).val();c.mapLayers[d].setOpacity(e[d]/100)});e.push(k);h.append(g)});var l=jQuery("#dialog-opacities").dialog({height:300,width:500,modal:!0,buttons:[{text:dlText.ok,click:function(){f=!1;l.dialog("close");c.bOp=d;e.forEach(function(a,d){c.lOps[d]=e[d]})}},{text:dlText.cancel,
...
e+'"]'),this.tUsed[e]=!0,h=c.vFrame.getSelLegend(e),g=PData.aByID(h),l=c.settings.iAtts[e],w=c.settings.cnt[e],f=c.sAtts[e],f=PData.aByID(f),f=PData.rTOrder(f,a,e),f.forEach(function(a){t=PData.rByN(a.i);u=t.a[h];"undefined"!==typeof u&&(k=PData.lRecs(u,g,n,!1))&&(c.rMap[a.i>>4]|=1<<(a.i&15),C=c.settings.lOn?'<div class="card-title">'+t.l+"</div>":"",m=!1,x="",w&&0<w.length&&(p=k.b?' style="color:black"':"",w.forEach(function(a){if(u=t.a[a])if(u=PData.procAttTxt(a,u))m=!0,x+=u+"<br/>"})),x=l&&(u=
t.a[l])?m?'<div class="card-body"><img src="'+u+'"/><div class="card-cnt"'+p+">"+x+"</div></div>":'<div class="card-body"><img class="full" src="'+u+'"/></div>':'<div class="card-body"><div class="card-cnt"'+p+">"+x+"</div>",z.append('<div class="card '+q+'" style="background-color:'+k.v+'" data-ai="'+a.i+'">'+C+x+"</div>"))}));f=a.t[++e]}};VizCards.prototype.teardown=function(){jQuery(this.frameID).off("click.vf")};
...
t.l+"</div>":"",w=!1,u="",l&&0<l.length&&(C=m.b?' style="color:black"':"",l.forEach(function(a){if(k=t.a[a])if(k=PData.procAttTxt(a,k))w=!0,u+=k+"<br/>"})),u=g&&(k=t.a[g])?w?'<div class="card-body"><img src="'+k+'"/><div class="card-cnt"'+C+">"+u+"</div></div>":'<div class="card-body"><img class="full" src="'+k+'"/></div>':'<div class="card-body"><div class="card-cnt"'+C+">"+u+"</div>",d.append('<div class="card '+p+(c.isSel(a.i)?" obj-sel":"")+'" style="background-color:'+m.v+'" data-ai="'+a.i+'">'+
...
VizPinboard.prototype.doOptions=function(){function a(){h&&(d3.select("#base-"+d).attr("opacity",c.bOp/100),c.lOps.forEach(function(a,c){d3.select("#ol-"+d+"-"+c).attr("opacity",a/100)}));g.empty()}var c=this,d=this.vFrame.getIndex(),e=this.bOp,f=[],h=!0,g=jQuery("#dialog-opacities div.layer-list"),l=jQuery('<div class="op-layer" data-i="-1">Base Image <input type=range class="op-slider" min=0 max=100 value='+this.bOp+" step=5></div>");l.find(".op-slider").on("change",function(){e=jQuery(this).val();
d3.select("#base-"+d).attr("opacity",e/100)});g.append(l);this.lOps.forEach(function(a,c){l=jQuery('<div class="op-layer" data-i="'+c+'">Overlay '+(c+1)+' <input type=range class="op-slider" min=0 max=100 value='+a+" step=5></div>");l.find(".op-slider").on("change",function(){f[c]=jQuery(this).val();d3.select("#ol-"+d+"-"+c).attr("opacity",f[c]/100)});f.push(a);g.append(l)});var n=jQuery("#dialog-opacities").dialog({height:300,width:320,modal:!0,buttons:[{text:dlText.ok,click:function(){h=!1;n.dialog("close");
...
VizTime.prototype.setup=function(){function a(a){var c={id:a,l:0,t:0,h:0,w:h[1],svgID:"#tl-b-"+l+"-"+a,tHt:0,iHt:0,xScale:d3.time.scale(),yScale:function(a){return a*c.tHt},parts:[],g:null,labels:null,labelSVGs:null,redraw:function(){}};a?(c.tHt=e.settings.bHt,c.iHt=c.tHt-2):(c.tHt=3,c.iHt=2);if(1==a){var k=e.minDate,m=e.maxDate;0<e.settings.zFrom.length&&(k=PData.dStr(e.settings.zFrom,!1));0<e.settings.zTo.length&&(m=PData.dStr(e.settings.zTo,!0));c.xScale.domain([k,m]);e.zMinDate=k;e.zMaxDate=m}else c.xScale.domain([e.minDate,
...
e.settings.xLbl).attr("height",k);var d=m.append("text").attr("class","bMinMaxLbl").attr("id",function(a){return"txt-"+a.name}).attr("x",function(a){return a.x()+a.tDelta}).attr("y",12).attr("text-anchor",function(a){return a.anchor}),u={redraw:function(){var a=c.xScale.domain(),k=a[0],m=a[1];d.text(function(a){return a.whichDate(k,m).getUTCFullYear()})}};c.parts.push(u);e.cmpnts.push(u);if(1===a){var f=m.append("text").attr("class","bMinMaxLbl").attr("id",function(a){return"m-txt-"+a.name}).attr("x",
function(a){return a.x()+a.tDelta}).attr("y",k-4).attr("text-anchor",function(a){return a.anchor});a={redraw:function(){var a=c.xScale.domain(),k=a[0],m=a[1];f.text(function(a){return m.getUTCFullYear()-k.getUTCFullYear()>e.threshold?"":months[a.whichDate(k,m).getMonth()]})}};c.parts.push(a);e.cmpnts.push(a)}c.labelSVGs=m}var e=this;"number"!==typeof this.settings.xLbl&&(this.settings.xLbl=parseInt(this.settings.xLbl));this.brushSVG=this.brush=null;var f=this.settings;"string"===typeof f.bHt&&(f.bHt=
...
VizDirectory.prototype.rerender=function(a){var c=this,d,e,f,h,g,l=jQuery(this.frameID+' table.viz-directory[data-ti="'+a+'"] tbody');l.empty();tRec=this.stream.t[a];d=c.settings.cnt[a];e=PData.aByID(c.sAtts[a]);PData.rTOrder(e,c.stream,a).forEach(function(a){h=PData.rByN(a.i);g="<tr "+(c.isSel(a.i)?'class="obj-sel" ':"")+'data-id="'+h.id+'" data-ai='+a.i+">";d.forEach(function(a){f=h.a[a];"undefined"!==typeof f?g=(f=PData.procAttTxt(a,f))?g+("<td>"+f+"</td>"):g+"<td></td>":g+="<td></td>"});l.append(g+
...
VizTextStream.prototype.render=function(a){var c=this,d=PData.eTNum(),e=0,f,h,g,l,n,t,k,m,w,u,x,C,p,y,z,q,v,r=jQuery(this.frameID);r.empty();0<this.recSel.length&&(this.recSel=[]);this.preRender();q=this.settings.max-this.settings.min;for(h=a.t[0];e<d;){for(;0===h.n;){if(++e===d)return;h=a.t[e]}w=c.vFrame.getSelLocAtts(e);if(0!==w.length&&(w=c.vFrame.getSelFeatAtts(e),0!==w.length)){c.tUsed[e]=!0;u=c.vFrame.getSelLegend(e);x=PData.aByID(u);r.append('<div class="viz-textstream" data-ti='+e+"></div>");
g=r.find("div.viz-textstream[data-ti="+e+"]");f=PData.eTByN(e);f=PData.tByID(f);g.append('<div class="template-label">'+f.l+"</div>");m=c.settings.cnt[e];if(y=c.settings.sAtts[e])p=PData.aByID(y),"number"===typeof p.r.min&&"number"===typeof p.r.max?z=p.r.max-p.r.min:y=null;m&&(f=PData.aByID(c.settings.order[e]),f=PData.rTOrder(f,a,e),f.forEach(function(a){l=PData.rByN(a.i);n=l.a[u];"undefined"!==typeof n&&(C=PData.lRecs(n,x,w,!1))&&((t=l.a[m])&&(t=PData.procAttTxt(m,t)),t&&(c.rMap[a.i>>4]|=1<<(a.i&
15),y?(k=l.a[y],k="number"===typeof k?Math.floor((k-p.r.min)*q/z)+c.settings.min:c.settings.min):k=c.settings.min,v=C.b?";background-color:black":"",g.append('<div class="recitem" data-ai='+a.i+' style="color:'+C.v+v+";font-size:"+k+'px">'+t+"</div>")))}))}e++}};VizTextStream.prototype.teardown=function(){jQuery(this.frameID).off("click.vf")};
...
VizNetWheel.prototype.render=function(a){function c(a){a=e.toggleSel(a.ai);d3.select(this).classed("obj-sel",a)}function d(a){PState.set(PSTATE_UPDATE);l.each(function(a){a.linked=!1});h.each(function(c){if(c.s===a){c.t.linked=!0;for(var k=0;k<f.length;k++){var d=f[k];if(d.source===a&&d.target===c.t){d3.select(this).attr("stroke",d.c).classed("thick",!0);this.parentElement.appendChild(this);break}}}else if(c.t===a)for(c.s.linked=!0,k=0;k<f.length;k++){if(d=f[k],d.target===a&&d.source===c.s){d3.select(this).attr("stroke",
d.c).classed("thick",!0);this.parentElement.appendChild(this);break}}else d3.select(this).attr("stroke","black").classed("thick",!1)});l.select("text").attr("fill",function(a){return a.linked?"white":"black"});d3.select(this).attr("fill","yellow");PState.set(PSTATE_READY)}var e=this,f=[],h,g,l;a?this.stream=a:a=this.stream;this.svg.selectAll(".node").remove();this.svg.selectAll(".link").remove();0<this.recSel.length&&(this.recSel=[],this.vFrame.selBtns(!1));this.preRender();if(0===a.l)this.svg.attr("width",
"10").attr("height","10");else{var n={children:[]};(function(){var c,k=0,d=0,f,h,p,g=[],l=null,q,t;c=a.t[0];t=e.vFrame.getSelLegend(0);q=PData.aByID(t);l=e.vFrame.getSelFeatAtts(0);0<l.length&&0<c.n&&(e.tUsed[0]=!0);a:for(;d<a.l;){for(;0===c.n||c.i+c.n===d||0===l.length;){0<g.length&&(n.children.push({ti:k,children:g}),g=[]);if(++k===PData.eTNum())break a;g=[];c=a.t[k];t=e.vFrame.getSelLegend(k);q=PData.aByID(t);l=e.vFrame.getSelFeatAtts(k);0<l.length&&0<c.n&&(e.tUsed[k]=!0)}p=a.s[d];f=PData.rByN(p);
h=f.a[t];"undefined"!==typeof h&&(fData=PData.lRecs(h,q,l,!1))&&(e.rMap[p>>4]|=1<<(p&15),g.push({r:f,ai:p,c:fData.v,l:!1,children:[]}));d++}0<g.length&&n.children.push({ti:k,children:g})})();n.children.forEach(function(a){var c=e.settings.pAtts[a.ti],k;a.children.forEach(function(a){c.forEach(function(c){k=a.r.a[c.pid];"undefined"!==typeof k&&k.forEach(function(k){var d=!1,m,e=0;a:for(;e<n.children.length;e++)for(var w=n.children[e],u=0;u<w.children.length;u++)if(m=w.children[u],m.r.id==k){d=!0;break a}d&&
(a.l=!0,m.l=!0,f.push({source:a,target:m,c:c.clr}))})})})});this.prune&&n.children.forEach(function(a){for(var c=a.children.length-1;0<=c;c--){var k=a.children[c];k.l||(e.rMap[k.ai>>4]&=1<<(k.ai&15)^65535,a.children.splice(c,1))}});var t=0;n.children.forEach(function(a){t+=a.children.length});this.inc=360/(t+n.children.length);if(0===t)this.svg.attr("width","10").attr("height","10");else{g=Math.max((14*t+20)/(2*Math.PI),30);var k=this.settings.lw;"string"===typeof k&&(k=parseInt(k));this.cr=k+12+
g;k=2*this.cr;this.svg.attr("width",k).attr("height",k);this.center.attr("transform","translate("+this.cr+","+this.cr+")");g=d3.layout.cluster().size([360,g]).sort(null).nodes(n);l=this.center.append("g").selectAll(".node").data(g.filter(function(a){return!a.children})).enter().append("g").attr("class","node").attr("transform",function(a){return"rotate("+(a.x-90)+")translate("+(a.y+8)+",0)"});l.append("circle").attr("r","5").style("fill",function(a){return a.c}).on("click",c);l.append("text").attr("dy",
".31em").attr("dx",function(a){return 180>a.x?"10":"-10"}).attr("transform",function(a){return 180>a.x?"":"rotate(180)"}).style("text-anchor",function(a){return 180>a.x?"start":"end"}).attr("fill","black").text(function(a){return a.r.l}).on("click",d);g=d3.layout.bundle();k=d3.svg.line.radial().interpolate("bundle").tension(.85).radius(function(a){return a.y}).angle(function(a){return a.x/180*Math.PI});h=this.center.append("g").selectAll(".link").data(g(f)).enter().append("path").each(function(a){a.s=
a[0];a.t=a[a.length-1]}).attr("class","link").attr("d",k).attr("stroke","black")}}};VizNetWheel.prototype.teardown=function(){var a=this.vFrame.getIndex();jQuery("#view-frame-"+a+" div.view-controls div.iconbar").remove()};VizNetWheel.prototype.setSel=function(a){var c=this;c.recSel=a;this.svg.selectAll(".node circle").attr("class",function(a){return c.isSel(a.ai)?"obj-sel":""})};
VizNetWheel.prototype.clearSel=function(){0<this.recSel.length&&(this.recSel=[],this.svg.selectAll(".node circle").attr("class",""))};
...
PFilterDates.prototype.evalBoxes=function(a){function c(c,h){function g(k){m=!0;a.find(c+k).addClass("error")}function k(k){a.find(c+k).removeClass("error")}var m=!1,w={y:0,m:0,d:0,ms:null},u=a.find(c+"-y"),u=u.val();(u=e.exec(u))?(k("-y"),w.y=parseInt(u[1],10)):g("-y");u=a.find(c+"-m");u=u.val();(u=f.exec(u))?(w.m=parseInt(u[1],10),1>w.m||12<w.m?g("-m"):k("-m")):g("-m");u=a.find(c+"-d");u=u.val();(u=f.exec(u))?(w.d=parseInt(u[1],10),1>w.d||31<w.d?g("-d"):k("-d")):g("-d");m||(w.ms=PData.d3Nums(w.y,
w.m,w.d,!1),0==h?w.ms<d.rCats[0].min&&(g("-y"),g("-m"),g("-d")):w.ms>d.rCats[d.rCats.length-1].max&&(g("-y"),g("-m"),g("-d")));d.uDates[h]=w;return m}var d=this,e=/^(-?\d+)$/,f=/^(\d{1,2})$/,h,g;h=c(".from",0);g=c(".to",1);this.uDates[0].ms>this.uDates[1].ms&&(g=!0);a.find(".filter-update").prop("disabled",h||g);return!(h||g)};
...
case "t":return c;case "P":if(0<c.length){var g="";c.forEach(function(a){if(a=PData.rByID(a))0<g.length&&(g+=", "),g+='<a href="'+prspdata.site_url+"?p="+a.wp+'" target="_blank">'+a.l+"</a>"});return g}}return null},rAV:function(a,c,d){for(var e=0;e<l.length;e++){var f=l[e];if(0<f.n&&f.i<=a&&a<f.i+f.n){a=f.d[a-f.i].a[c];if(null==a||"undefined"===typeof a)break;return d?a:PData.procAttTxt(c,a)}}return null},nByID:function(a){for(var c=0;c<l.length;c++){var d=l[c];if(0<d.n)for(var e=0,f=d.n-1,g,p;e<=
