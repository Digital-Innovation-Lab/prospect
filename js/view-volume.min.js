...
4]&1<<(a&15)&&(f=PData.rByN(a),b.eval(f)&&m.push(a))}b.evalDone(r.l)}PState.set(PSTATE_UPDATE);0<m.length?(d.setSel(m),"v2"!==x&&c.setSel(m)):(d.clearSel(),"v2"!==x&&c.clearSel())}PState.set(PSTATE_READY)}function J(a,b){var c;c=jQuery("#dialog-hilite-"+a).dialog({height:275,width:Math.min(jQuery(window).width()-20,675),modal:!0,appendTo:"#dialog-1",buttons:[{text:dlText.chsatt,click:function(){E(!1,!0,b,function(b){p[a]=b;t[a]=G(b,null,a)})}},{text:dlText.ok,click:function(){c.dialog("close");null!==
...
b.l})}(a.s.v1.l);h?(h.setViz(vI,!0),h.selBtns(!1)):(n[1]=new PVizFrame(1,H),h=n[1],h.initDOM(vI));h.setState(a.s.v1.s);k(a.n);null!=a.s.h0?e(0,a.s.h0):null!=a.s.h1?(e(1,a.s.h1),g.clearSel()):(p[0]=p[1]=t[0]=t[1]=null,g.setSel(f(a.s.recs[0])),"v2"!==x?(h.setSel(f(a.s.recs[1])),F=null):F=a.s.recs[1]);PData.ready()&&A&&(c(),null!=p[0]&&z(0),C(),null!=p[1]?z(1):null!=F&&(h.setSel(f(F)),F=null));return!0}function C(){var a=n[0],b=n[1];if(null!=A)switch(x){case "v0":r=A;b.showStream(A);a=a.vizSel;0<a.length?
...
var a=b.find("li.selected");a.length&&(a=a.data("id"),D(a),PState.set(PSTATE_READY))}},{text:dlText.cancel,click:function(){d.dialog("close")}}];y&&c.push({text:dlText.manage,click:function(){d.dialog("close");h()}});var d=jQuery("#dialog-show-reading").dialog({width:350,height:350,modal:!0,buttons:c});a.preventDefault()});jQuery("#btn-save-reading").button({icons:{primary:"ui-icon-pencil"},text:!1}).click(function(a){var c,d=/[^\w\-]/;jQuery("#save-reading-id").val("");jQuery("#save-reading-lbl").val("");
