...
0;I<H.length&&!(a.e<H[I]);I++);a.t=I+p+1;H[I]=a.s});h=PData.getAttID(f);h.l.forEach(function(a){B=a.d;A=B.min.y;"undefined"==typeof B.min.m?D=r=1:(r=B.min.m,D="undefined"==typeof B.min.d?1:B.min.d);w=PData.date3Nums(A,r,D,!1);"undefined"==typeof B.max.y?x=TODAY:(A=B.max.y,"undefined"==typeof B.max.m?(r=12,D=31):(r=B.max.m,D="undefined"==typeof B.max.d?mnthDays[r-1]:B.max.d),x=PData.date3Nums(A,r,D,!0));g.lgBds.push({s:w,e:x,t:p,h:H.length+1,d:a})});p+=H.length+1;g.events=g.events.concat(G)}e++}})();
g.getWidths();e(0);e(1);(function(){var a=function(){function a(c,h){this._height=h;this.g=c;this.mask=this.g.attr("class","brush").call(g.brush);this.mask.selectAll("rect").attr("y",0).attr("height",h);this.left=this.mask.append("polygon").attr("class","dragger");this.right=this.mask.append("polygon").attr("class","dragger");this._bottom=this._top=this._x=null}a.prototype.style=function(a,c){this.left.style(a,c);this.right.style(a,c);return this};a.prototype.x=function(a){if(null==a)return this._x;
this._x=a;return this};a.prototype.height=function(a){if(null==a)return this._height;this._height=a;this.mask.selectAll("rect").attr("height",a);return this};a.prototype.redraw=function(){var a,c;c=g.brush.extent();this._x.range();a=c[0];c=c[1];a=this._x(a);c=this._x(c);this.left.attr("points",""+a+","+this._height+" "+(a+5)+","+(this._height+5)+" "+(a+5)+","+(this._height+9)+" "+(a-5)+","+(this._height+9)+" "+(a-5)+","+(this._height+5)+" "+a+","+this._height);this.right.attr("points",""+c+","+this._height+
" "+(c+5)+","+(this._height+5)+" "+(c+5)+","+(this._height+9)+" "+(c-5)+","+(this._height+9)+" "+(c-5)+","+(this._height+5)+" "+c+","+this._height);return this};return a}(),c=g.bands[0];null!=g.brushSVG&&g.brushSVG.remove();g.brush=d3.svg.brush().x(c.xScale.range([0,c.w])).extent([g.zMinDate,g.zMaxDate]).on("brush",function(){var a=g.brush.extent(),c;"move"===d3.event.mode?(c=d3.time.day.round(a[0]),a=d3.time.day.offset(c,Math.round((a[1]-a[0])/864E5)),c=[c,a]):(c=a.map(d3.time.day.round),c[0]>=c[1]&&
