...
m.empty();var p=jQuery('<div class="op-layer" data-i="-1">Base Map <input type=range class="op-slider" min=0 max=100 value=100 step=5></div>');m.append(p);_.each(this.settings.lyrs,function(d,k){h=d.o||1;var r;r=PMapHub.createMapLayer(d.lid,h,a.lMap,null);a.mapLayers.push(r);p=jQuery('<div class="op-layer" data-i="'+k+'">'+r.options.layerName+' <input type=range class="op-slider" min=0 max=100 value='+100*d.o+" step=5></div>");m.append(p)});var n=_.template(document.getElementById("dltext-v-map").innerHTML);
jQuery("#view-frame-"+g+" div.view-control-bar").append(n({vi:g}));jQuery("#map-zoom-"+g).button({text:!1,icons:{primary:"ui-icon-plus"}}).click(function(){a.lMap.zoomIn()});jQuery("#map-unzoom-"+g).button({text:!1,icons:{primary:"ui-icon-minus"}}).click(function(){a.lMap.zoomOut()});jQuery("#map-reset-"+g).button({text:!1,icons:{primary:"ui-icon-arrowrefresh-1-w"}}).click(function(){a.lMap.setView([d,e],f)});jQuery("#map-cloc-"+g).button({text:!1,icons:{primary:"ui-icon-pin-s"}}).click(function(){navigator.geolocation.getCurrentPosition(function(d){a.lMap.setView([d.coords.latitude,
...
VizMap.prototype.render=function(a){function d(a){if(a.target&&a.target.options){var d=a.target.options._aid,k=e.toggleSel(d);a=PData.aIndex2Tmplt(d);1<e.tLCnt[a]?(PState.set(PSTATE_UPDATE),f.eachLayer(function(a){a.options._aid==d&&(k?a.setStyle({color:"yellow",weight:2}):a.setStyle({color:"#000",weight:1}))}),PState.set(PSTATE_READY)):k?this.setStyle({color:"yellow",weight:2}):this.setStyle({color:"#000",weight:1})}}var e=this,f=this.markerLayer;0<this.recSel.length&&(this.recSel=[]);f.clearLayers();
var g=this.lineLayer;g.clearLayers();var h=PData.getNumETmplts(),m=0,p,n=0,t,k,r,y,u,q,v,l,z,A,B,E,w,x,C,D,F;x=this.settings.min;"string"==typeof x&&(x=parseInt(x));n=this.settings.max;"string"==typeof n&&(n=parseInt(n));C=n-x;for(m=0;m<h;m++)this.tLCnt[m]=0;for(var H,m=0;m<h;m++)if("disable"!=this.settings.pAtts[m]){H=[];break}m=0;n=-1;a:for(;m<a.l;){if(null==q){do{if(++n==h)break a;t=a.t[n]}while(0==t.n||t.i+t.n==m);q=this.vFrame.getSelLocAtts(n);if(0==q.length){q=null;continue}v=e.vFrame.getSelFeatAtts(n);
if(0==v.length){q=null;continue}e.tLCnt[n]=q.length;y=e.vFrame.getSelLegend(n);u=PData.getAttID(y);l=e.settings.pAtts[n];k=e.settings.lClrs[n];if(E=e.settings.sAtts[n])w=PData.getAttID(E),"number"==typeof w.r.min&&"number"==typeof w.r.max?(D=w.r.min,F=w.r.max-D):E=null}p=a.s[m];r=PData.getRecByIndex(p);var G;H&&(G={id:r.id,c:[],p:r.a[l],l:k});q.forEach(function(a){if(z=r.a[a])if(A=r.a[y],"undefined"!=typeof A&&(A=PData.getAttLgndVal(A,u,v))){if("number"==typeof z[0])E?(w=r.a[E],w="undefined"!=typeof w?
Math.floor((w-D)*C/F)+x:x):w=x,B=L.circleMarker(z,{_aid:p,weight:1,radius:w,fillColor:A,color:"#000",opacity:1,fillOpacity:1}),G&&G.c.push(z);else if(B=2==z.length?L.polyline(z,{_aid:p,weight:1,fillColor:A,color:"#000",opacity:1,fillOpacity:1}):L.polygon(z,{_aid:p,weight:1,fillColor:A,color:"#000",opacity:1,fillOpacity:1}),G){var k=[0,0];z.forEach(function(a){k[0]+=a[0];k[1]+=a[1]});k[0]/=z.length;k[1]/=z.length;G.c.push(k)}B.on("click",d);f.addLayer(B)}});G&&0<G.c.length&&H.push(G);++m==t.i+t.n&&
(q=null)}if(H){H.sort(function(a,d){return PData.strcmp(d.id,a.id)});var I=[];H.forEach(function(a){a.p&&a.p.forEach(function(d){m=_.sortedIndex(H,{id:d},"id");if(m<H.length){var k=H[m];k.id==d&&a.c.forEach(function(d){k.c.forEach(function(k){d[0]==k[0]&&d[1]==k[1]||I.push({p:[d,k],c:a.l})})})}})});I.forEach(function(a){g.addLayer(L.polyline(a.p,{color:a.c,weight:2}))})}};VizMap.prototype.teardown=function(){var a=this.vFrame.getIndex();jQuery("#view-frame-"+a+" div.view-control-bar div.iconbar").remove()};
...
d+'"]').val(this.sAtts[d])};
VizCards.prototype.render=function(a){var d=this;this.stream=a;var e=PData.getNumETmplts(),f,g,h,m,p,n,t,k,r,y,u,q,v,l,z=jQuery(this.frameID);z.empty();0<this.recSel.length&&(this.recSel=[]);var A,B="w"+this.settings.w+" h"+this.settings.h;g=a.t[0];for(f=0;f<e;){for(;0==g.n;){if(++f==e)return;g=a.t[f]}g=PData.getETmpltIndex(f);g=PData.getTmpltID(g);n=d.vFrame.getSelFeatAtts(f);0!=n.length&&(z.append('<div class="template-label">'+g.l+'</div><div class="cards" data-ti="'+f+'"></div>'),A=jQuery('div.cards[data-ti="'+
f+'"]'),h=d.vFrame.getSelLegend(f),m=PData.getAttID(h),p=d.settings.iAtts[f],y=d.settings.cnt[f],g=d.sAtts[f],g=PData.getAttID(g),g=PData.orderTBy(g,a,f),g.forEach(function(a){t=PData.getRecByIndex(a.i);u=t.a[h];"undefined"!=typeof u&&(k=PData.getAttLgndRecs(u,m,n,!1))&&(v=d.settings.lOn?'<div class="card-title">'+t.l+"</div>":"",r=!1,q="",y&&0<y.length&&(l=k.b?' style="color:black"':"",y.forEach(function(a){if(u=t.a[a])if(u=PData.procAttTxt(a,u))r=!0,q+=u+"<br/>"})),q=p&&(u=t.a[p])?r?'<div class="card-body"><img src="'+
u+'"/><div class="card-cnt"'+l+">"+q+"</div></div>":'<div class="card-body"><img class="full" src="'+u+'"/></div>':'<div class="card-body"><div class="card-cnt"'+l+">"+q+"</div>",A.append('<div class="card '+B+'" style="background-color:'+k.v+'" data-ai="'+a.i+'">'+v+q+"</div>"))}));g=a.t[++f]}};VizCards.prototype.teardown=function(){jQuery(this.frameID).off("click.vf")};
VizCards.prototype.rerender=function(a){var d=this,e=jQuery(this.frameID+' div.cards[data-ti="'+a+'"]');e.empty();var f,g,h,m,p,n,t,k,r,y,u,q,v,l="w"+this.settings.w+" h"+this.settings.h;f=d.vFrame.getSelFeatAtts(a);0!=f.length&&(g=d.vFrame.getSelLegend(a),h=PData.getAttID(g),m=d.settings.iAtts[a],p=d.settings.cnt[a],n=d.sAtts[a],n=PData.getAttID(n),a=PData.orderTBy(n,d.stream,a),a.forEach(function(a){t=PData.getRecByIndex(a.i);k=t.a[g];"undefined"!=typeof k&&(r=PData.getAttLgndRecs(k,h,f,!1))&&(q=
d.settings.lOn?'<div class="card-title">'+t.l+"</div>":"",y=!1,u="",p&&0<p.length&&(v=r.b?' style="color:black"':"",p.forEach(function(a){if(k=t.a[a])if(k=PData.procAttTxt(a,k))y=!0,u+=k+"<br/>"})),u=m&&(k=t.a[m])?y?'<div class="card-body"><img src="'+k+'"/><div class="card-cnt"'+v+">"+u+"</div></div>":'<div class="card-body"><img class="full" src="'+k+'"/></div>':'<div class="card-body"><div class="card-cnt"'+v+">"+u+"</div>",e.append('<div class="card '+l+(d.isSel(a.i)?" obj-sel":"")+'" style="background-color:'+
...
VizPinboard.prototype.render=function(a){var d=this;this.gRecs.selectAll(".recobj").remove();this.gRecs.selectAll(".recline").remove();0<this.recSel.length&&(this.recSel=[]);var e=PData.getNumETmplts(),f,g,h=0,m,p,n,t,k,r,y,u,q,v,l,z,A,B,E,w;for(f=0;f<e;f++)if("disable"!=this.settings.pAtts[f]){w=[];break}z=this.settings.min;"string"==typeof z&&(z=parseInt(z));g=this.settings.max;"string"==typeof g&&(g=parseInt(g));A=g-z;var x=[];f=0;h=-1;a:for(;f<a.l;){if(null==r){do{if(++h==e)break a;m=a.t[h]}while(0==
m.n||m.i+m.n==f);r=this.vFrame.getSelLocAtts(h);if(0==r.length){r=null;continue}r=r[0];y=d.vFrame.getSelFeatAtts(h);if(0==y.length){r=null;continue}t=d.vFrame.getSelLegend(h);k=PData.getAttID(t);u=d.settings.pAtts[h];p=d.settings.lClrs[h];if(l=d.settings.sAtts[h])n=PData.getAttID(l),"number"==typeof n.r.min&&"number"==typeof n.r.max?(B=n.r.min,E=n.r.max-B):l=null}g=a.s[f];n=PData.getRecByIndex(g);var C;w&&(C={id:n.id,c:null,p:n.a[u],l:p});q=n.a[r];"undefined"!=typeof q&&(v=n.a[t],"undefined"!=typeof v&&
(v=PData.getAttLgndVal(v,k,y)))&&(l?(n=n.a[l],n="undefined"!=typeof n?Math.floor((n-B)*A/E)+z:z):n=z,x.push({ai:g,v:v,x:q[0],y:q[1],r:n}),C&&(C.c=q));C&&C.c&&w.push(C);++f==m.i+m.n&&(r=null)}this.gRecs.selectAll(".recobj").data(x).enter().append("circle").attr("class","recobj").attr("cx",function(a){return d.xScale(a.x)}).attr("cy",function(a){return d.yScale(a.y)}).attr("r",function(a){return d.yScale(a.r)}).style("fill",function(a){return a.v}).on("click",function(a,k){var r=d.toggleSel(a.ai);d3.select(this).classed("obj-sel",
r)});if(w){w.sort(function(a,d){return PData.strcmp(d.id,a.id)});var D=[];w.forEach(function(a){a.p&&a.p.forEach(function(d){f=_.sortedIndex(w,{id:d},"id");if(f<w.length){var k=w[f];k.id==d&&D.push({f:a.c,t:k.c,c:a.l})}})});this.gRecs.selectAll(".recline").data(D).enter().append("line").attr("class","recline").attr("x1",function(a){return d.xScale(a.f[0])}).attr("y1",function(a){return d.yScale(a.f[1])}).attr("x2",function(a){return d.xScale(a.t[0])}).attr("y2",function(a){return d.yScale(a.t[1])}).attr("stroke",
...
VizTime.prototype.setup=function(){function a(a){var d={id:a,l:0,t:0,h:0,w:h[1],svgID:"#tl-b-"+p+"-"+a,tHt:0,iHt:0,xScale:d3.time.scale(),yScale:function(a){return a*d.tHt},parts:[],g:null,labels:null,labelSVGs:null,redraw:function(){}};a?(d.tHt=f.settings.bHt,d.iHt=d.tHt-2):(d.tHt=3,d.iHt=2);if(1==a){var k=f.minDate,r=f.maxDate;0<f.settings.zFrom.length&&(k=PData.parseDate(f.settings.zFrom,!1));0<f.settings.zTo.length&&(r=PData.parseDate(f.settings.zTo,!0));d.xScale.domain([k,r]);f.zMinDate=k;f.zMaxDate=
r}else d.xScale.domain([f.minDate,f.maxDate]);d.xScale.range([0,d.w]);d.g=f.chart.append("g").attr("id",d.svgID.substring(1)).attr("class","tl-band").attr("width",d.w);f.bands[a]=d;f.cmpnts.push(d)}function d(a){var d=f.bands[a],k=d3.svg.axis().scale(d.xScale).orient("bottom").tickSize(6,0);localD3&&k.tickFormat(localD3);var r=d.g.append("g").attr("id","axis-"+p+"-"+a).attr("class","axis").style("font-size","10px");a={redraw:function(){r.call(k)}};d.parts.push(a);f.cmpnts.push(a)}function e(a){var d=
f.bands[a],k;k=1==a?32:16;d.labels=[{name:"s-"+p+"-"+a,x:function(){return 0},left:function(){return 0},anchor:"start",tDelta:2,whichDate:function(a,d){return a}},{name:"e-"+p+"-"+a,x:function(){return d.l+d.w},left:function(){return d.l+d.w-f.settings.xLbl},anchor:"end",tDelta:-3,whichDate:function(a,d){return d}}];var r=d3.select(d.svgID).selectAll(".bLblCntr").data(d.labels).enter().append("g").attr("class","bLblCntr");r.append("rect").attr("class","bLbl").attr("id",function(a){return"rect-"+a.name}).attr("x",
...
a);f.append("stop").attr("offset","100%").attr("stop-color","#C0C0C0");f=k.append("linearGradient").attr("id",e+"-fb");f.append("stop").attr("offset","0%").attr("stop-color","#C0C0C0");f.append("stop").attr("offset","5%").attr("stop-color",a);f.append("stop").attr("offset","95%").attr("stop-color",a);f.append("stop").attr("offset","100%").attr("stop-color","#C0C0C0")})})();var p=this.vFrame.getIndex();m.append("clipPath").attr("id","tl-clip-"+p).append("rect").attr("width",h[1]);f.chart=m.append("g").attr("class",
"chart").attr("clip-path","url(#tl-clip-"+p+")");f.instRad=g.bHt/2-1;f.bands=Array(2);f.cmpnts=[];a(0);a(1);d(0);d(1);e(0);e(1)};
VizTime.prototype.render=function(a){function d(a,d){var k=a.s-d.s;if(0>k)return 1;if(0<k)return-1;k=d.e-a.e;return 0>k?1:0<k?-1:0}function e(a){function d(a){a=h.toggleSel(a.ai);d3.select(this).classed("obj-sel",a)}var k,f,e,g,q=h.bands[a];a?(k=h.bands[0],q.t=k.t+k.h+37,k=f=e=h.instRad,g=2*h.instRad+3):(q.t=0,k=f=e=1);q.h=p*q.tHt+2;q.g.attr("transform","translate(0,"+q.t+")").attr("height",q.h);var v,l;a&&(v=.75*q.iHt+"px",l=.8*q.iHt);var m;m=d3.select(q.svgID).selectAll(".lgBd").remove();m=d3.select(q.svgID).selectAll(".lgBd").data(h.lgBds).enter().append("svg").attr("class",
"lgBd").attr("y",function(a){return q.yScale(a.t)}).attr("height",function(a){return q.yScale(a.h)});m.append("rect").attr("width","100%").attr("height","100%").attr("fill",function(a){return a.d.v});1==a&&m.append("text").attr("class","lgBdLbl").attr("x",2).attr("y",l).attr("fill",function(a){return a.d.b?"#000000":"#FFFFFF"}).style("font-size",v).text(function(a){return a.d.l});var A;d3.select(q.svgID).selectAll(".event").remove();A=d3.select(q.svgID).selectAll(".event").data(h.events).enter().append("svg").attr("class",
...
a&&B.append("text").attr("class","rangeLbl").attr("x",4).attr("y",l).attr("fill",function(a){return a.c.b?"#000000":"#FFFFFF"}).style("font-size",v).text(function(a){return a.l});B=d3.select(q.svgID).selectAll(".instant");B.append("circle").attr("cx",k).attr("cy",f).attr("r",e).attr("fill",function(a){return a.c.v});1==a&&B.append("text").attr("class","instantLbl").attr("x",g).attr("y",l).style("font-size",v).text(function(a){return a.l});q.redraw=function(){m.attr("x",function(a){return q.xScale(a.s)}).attr("width",
...
h.events=[];h.lgBds=[];var p=0;(function(){for(var f=PData.getNumETmplts(),e=0,k,r,y,g,q,v,l,m;e<f;){for(k=a.t[e];0==k.n;){if(++e==f)return;k=a.t[e]}y=h.vFrame.getSelLocAtts(e);if(0!=y.length&&(y=h.vFrame.getSelFeatAtts(e),0!=y.length)){m=h.vFrame.getSelLegend(e);l=PData.getAttID(m);g=h.settings.dAtts[e];for(var A,B,E,w,x,C,D,F,H=[],G=k.i;G<k.i+k.n;G++)r=a.s[G],F=PData.getRecByIndex(r),q=F.a[m],"undefined"!=typeof q&&(q=PData.getAttLgndRecs(q,l,y,!1))&&(v=F.a[g])&&(C=v.min.f?EVENT_F_START:0,A=v.min.y,
"undefined"==typeof v.min.m?E=B=1:(B=v.min.m,E="undefined"==typeof v.min.d?1:v.min.d),w=PData.date3Nums(A,B,E,!1),"undefined"==typeof v.max?(C|=EVENT_INSTANT,x=w.getTime()+h.instGap):"open"==v.max?x=TODAY:(v.max.f&&(C|=EVENT_F_END),A=v.max.y,"undefined"==typeof v.max.m?(B=12,E=31):(B=v.max.m,E="undefined"==typeof v.max.d?PData.lenMnth(A,B):v.max.d),x=PData.date3Nums(A,B,E,!0)),H.push({s:w,e:x,ai:r,f:C,c:q,l:F.l,t:0}));H.sort(d);var I=[],M;H.forEach(function(a){for(M=0;M<I.length&&!(a.e<I[M]);M++);
a.t=M+p+1;I[M]=a.s});k=PData.getAttID(g);k.l.forEach(function(a){D=a.d;A=D.min.y;"undefined"==typeof D.min.m?E=B=1:(B=D.min.m,E="undefined"==typeof D.min.d?1:D.min.d);w=PData.date3Nums(A,B,E,!1);"undefined"==typeof D.max.y?x=TODAY:(A=D.max.y,"undefined"==typeof D.max.m?(B=12,E=31):(B=D.max.m,E="undefined"==typeof D.max.d?PData.lenMnth(A,B):D.max.d),x=PData.date3Nums(A,B,E,!0));h.lgBds.push({s:w,e:x,t:p,h:I.length+1,d:a})});p+=I.length+1;h.events=h.events.concat(H)}e++}})();h.getWidths();e(0);e(1);
...
VizDirectory.prototype.render=function(a){var d=PData.getNumETmplts(),e=0,f,g,h,m,p,n,t=jQuery(this.frameID);t.empty();0<this.recSel.length&&(this.recSel=[]);this.stream=a;for(f=a.t[0];e<d;){for(;0==f.n;){if(++e==d)return;f=a.t[e]}f=PData.getETmpltIndex(e);f=PData.getTmpltID(f);t.append('<div class="template-label">'+f.l+'</div><table cellspacing="0" class="viz-directory" data-ti='+e+"></table>");g=t.find('table[data-ti="'+e+'"]');h=this.settings.cnt[e];n="<thead><tr>";h.forEach(function(a){a=PData.getAttID(a);
n+="<th>"+a.def.l+"</th>"});g.append(n+"<tr></thead><tbody></tbody>");g=g.find("tbody");f=this.sAtts[e];f=PData.getAttID(f);f=PData.orderTBy(f,this.stream,e);f.forEach(function(a){p=PData.getRecByIndex(a.i);n='<tr data-id="'+p.id+'" data-ai='+a.i+">";h.forEach(function(a){m=p.a[a];"undefined"!=typeof m?n=(m=PData.procAttTxt(a,m))?n+("<td>"+m+"</td>"):n+"<td></td>":n+="<td></td>"});g.append(n+"</tr>")});f=a.t[++e]}};
VizDirectory.prototype.rerender=function(a){var d=this,e,f,g,h,m,p=jQuery(this.frameID+' table.viz-directory[data-ti="'+a+'"] tbody');p.empty();tRec=this.stream.t[a];e=d.settings.cnt[a];f=PData.getAttID(d.sAtts[a]);PData.orderTBy(f,d.stream,a).forEach(function(a){h=PData.getRecByIndex(a.i);m="<tr "+(d.isSel(a.i)?'class="obj-sel" ':"")+'data-id="'+h.id+'" data-ai='+a.i+">";e.forEach(function(a){g=h.a[a];"undefined"!=typeof g?m=(g=PData.procAttTxt(a,g))?m+("<td>"+g+"</td>"):m+"<td></td>":m+="<td></td>"});
p.append(m+"</tr>")})};VizDirectory.prototype.teardown=function(){jQuery(this.frameID).off("click.vf")};
...
VizTextStream.prototype.render=function(a){var d=this,e=PData.getNumETmplts(),f=0,g,h,m,p,n,t,k,r,y,u,q,v,l,z,A,B,E,w=jQuery(this.frameID);w.empty();0<this.recSel.length&&(this.recSel=[]);B=this.settings.max-this.settings.min;for(h=a.t[0];f<e;){for(;0==h.n;){if(++f==e)return;h=a.t[f]}y=d.vFrame.getSelLocAtts(f);if(0!=y.length&&(y=d.vFrame.getSelFeatAtts(f),0!=y.length)){u=d.vFrame.getSelLegend(f);q=PData.getAttID(u);w.append('<div class="viz-textstream" data-ti='+f+"></div>");m=w.find("div.viz-textstream[data-ti="+
f+"]");g=PData.getETmpltIndex(f);g=PData.getTmpltID(g);m.append('<div class="template-label">'+g.l+"</div>");r=d.settings.cnt[f];if(z=d.settings.sAtts[f])l=PData.getAttID(z),"number"==typeof l.r.min&&"number"==typeof l.r.max?A=l.r.max-l.r.min:z=null;r&&(g=PData.getAttID(d.settings.order[f]),g=PData.orderTBy(g,a,f),g.forEach(function(a){p=PData.getRecByIndex(a.i);n=p.a[u];"undefined"!=typeof n&&(v=PData.getAttLgndRecs(n,q,y,!1))&&((t=p.a[r])&&(t=PData.procAttTxt(r,t)),t&&(z?(k=p.a[z],k="undefined"!=
typeof k?Math.floor((k-l.r.min)*B/A)+d.settings.min:d.settings.min):k=d.settings.min,E=v.b?";background-color:black":"",m.append('<div class="recitem" data-ai='+a.i+' style="color:'+v.v+E+";font-size:"+k+'px">'+t+"</div>")))}))}f++}};VizTextStream.prototype.teardown=function(){jQuery(this.frameID).off("click.vf")};
...
VizNetWheel.prototype.render=function(a){var d=this,e=[],f,g,h;this.svg.selectAll(".node").remove();this.svg.selectAll(".link").remove();this.inc=360/(a.l+a.t.length);g=Math.max((14*a.l+20)/(2*Math.PI),30);var m=this.settings.lw;"string"==typeof m&&(m=parseInt(m));this.cr=m+12+g;m=2*this.cr;this.svg.attr("width",m).attr("height",m);this.center.attr("transform","translate("+this.cr+","+this.cr+")");0<this.recSel.length&&(this.recSel=[]);var p={children:[]};(function(){var f,e=0,k=0,r,y,g,q=[],h=null,
l,m;f=a.t[0];m=d.vFrame.getSelLegend(0);l=PData.getAttID(m);h=d.vFrame.getSelFeatAtts(0);a:for(;k<a.l;){for(;0==f.n||f.i+f.n==k||0==h.length;){0<q.length&&(p.children.push({ti:e,children:q}),q=[]);if(++e==PData.getNumETmplts())break a;q=[];f=a.t[e];m=d.vFrame.getSelLegend(e);l=PData.getAttID(m);h=d.vFrame.getSelFeatAtts(e)}g=a.s[k];r=PData.getRecByIndex(g);y=r.a[m];"undefined"!=typeof y&&(fData=PData.getAttLgndRecs(y,l,h,!1))&&q.push({r:r,ai:g,c:fData.v,children:[]});k++}0<q.length&&p.children.push({ti:e,
children:q})})();g=d3.layout.cluster().size([360,g]).sort(null).nodes(p);h=this.center.append("g").selectAll(".node").data(g.filter(function(a){return!a.children})).enter().append("g").attr("class","node").attr("transform",function(a){return"rotate("+(a.x-90)+")translate("+(a.y+8)+",0)"});h.append("circle").attr("r","5").style("fill",function(a){return a.c}).on("click",function(a){a=d.toggleSel(a.ai);d3.select(this).classed("obj-sel",a)});h.append("text").attr("dy",".31em").attr("dx",function(a){return 180>
...
a)for(d.s.linked=!0,k=0;k<e.length;k++){if(f=e[k],f.target===a&&f.source===d.s){d3.select(this).attr("stroke",f.c).classed("thick",!0);this.parentElement.appendChild(this);break}}else d3.select(this).attr("stroke","black").classed("thick",!1)});h.select("text").attr("fill",function(a){return a.linked?"white":"black"});d3.select(this).attr("fill","yellow");PState.set(PSTATE_READY)});g=d3.layout.bundle();m=d3.svg.line.radial().interpolate("bundle").tension(.85).radius(function(a){return a.y}).angle(function(a){return a.x/
180*Math.PI});p.children.forEach(function(a){var f=d.settings.pAtts[a.ti],k;a.children.forEach(function(a){f.forEach(function(d){k=a.r.a[d.pid];"undefined"!=typeof k&&k.forEach(function(k){var f=!1,g,l=0;a:for(;l<p.children.length;l++)for(var h=p.children[l],m=0;m<h.children.length;m++)if(g=h.children[m],g.r.id==k){f=!0;break a}f&&e.push({source:a,target:g,c:d.clr})})})})});f=this.center.append("g").selectAll(".link").data(g(e)).enter().append("path").each(function(a){a.s=a[0];a.t=a[a.length-1]}).attr("class",
...
VizFlow.prototype.render=function(a){var d=this;this.bSel=[];this.fSel=[];this.barG.selectAll(".bar").remove();this.flowG.selectAll(".flow").remove();this.titleG.selectAll(".att-title").remove();var e=this.settings.w;d.bars=[];d.atts=[];d.settings.fcts.forEach(function(f,g){var h,m=PData.getAttID(f);"T"==m.def.t&&d.settings.tlit?(h=[],PData.fillCats(h,f,null,a,!0)):(h=d.settings.gr?PData.getRCats(m,!0):PData.getLCats(m,null),PData.fillCats(h,f,null,a,!1));var p=[],n=0;h.forEach(function(a){0<a.i.length&&
(p.push(a),n+=a.i.length)});var t=0,k=31+g*d.bH;p.forEach(function(a){d.bars.push({x:5+t*e/n,w:a.i.length*e/n,y:k,c:a});t+=a.i.length});d.atts.push({l:m.def.l,c:p,t:n,y:k})});this.barG.selectAll(".bar").data(d.bars).enter().append("rect").attr("class","bar").attr("x",function(a){return a.x}).attr("y",function(a){return a.y}).attr("fill",function(a){return a.c.c}).attr("height","8").attr("width",function(a){return a.w}).on("click",function(a,e){var h=d.bSel.length,m=_.sortedIndex(d.bSel,e);d.bSel[m]==
e?(d3.select(this).classed("obj-sel",!1),d.bSel.splice(m,1),0<h&&0==d.bSel.length&&0==d.fSel.length&&d.vFrame.selBtns(!1)):(d3.select(this).classed("obj-sel",!0),d.bSel.splice(m,0,e),0==h&&0==d.fSel.length&&0<d.bSel.length&&d.vFrame.selBtns(!0))}).append("title").text(function(a){return a.c.l+" ("+a.c.i.length+")"});d.ints=[];d.atts.forEach(function(a,g){if(g!=d.atts.length-1){for(var h=d.atts[g+1],m=Array(a.c.length),p=0;p<a.c.length;p++)m[p]=Array(h.c.length);a.c.forEach(function(a,d){h.c.forEach(function(k,
f){m[d][f]=PData.intersect(a.i,k.i)})});for(var n=Array(h.c.length),t=0;t<h.c.length;t++){for(var k=0,p=0;p<a.c.length;p++)0<m[p][t].length&&k++;n[t]=k-1}for(var r=Array(h.c.length),t=0;t<h.c.length;t++)r[t]=0;var y=0;a.c.forEach(function(k,q){for(var g=0,l=0;l<h.c.length;l++)0<m[q][l].length&&g++;--g;var t=k.i.length*e/a.t,p=0,B=0;h.c.forEach(function(l,w){var x=m[q][w];if(0<x.length){var C=x.length*e,D=C/a.t,C=C/h.t,F=l.i.length*e/h.t;d.ints.push({i:x,c:k.c,l:k.l+" > "+l.l,x1:5+y*e/a.t+(g?(t-D)*
p/g:0),x2:5+B*e/h.t+(n[w]?(F-C)*r[w]/n[w]:0),w1:D,w2:C,y1:a.y+10,y2:h.y-1});p++;r[w]++}B+=l.i.length});y+=k.i.length})}});this.flowG.selectAll(".flow").data(d.ints).enter().append("path").attr("class","flow").attr("d",function(a){return"M "+a.x1+" "+a.y1+"  L "+(a.x1+a.w1)+" "+a.y1+" L "+(a.x2+a.w2)+" "+a.y2+" L "+a.x2+" "+a.y2+" L "+a.x1+" "+a.y1}).attr("fill",function(a){return a.c}).on("click",function(a,e){var h=d.fSel.length,m=_.sortedIndex(d.fSel,e);d.fSel[m]==e?(d3.select(this).classed("obj-sel",
...
VizMBMap.prototype.renderTree=function(a){function d(){var a=0,d=e.settings.w,h=null!=e.sbkSel?e.sbkSel.i:e.bkSel.i;PState.set(PSTATE_UPDATE);e.facets.forEach(function(m,p){var n=[],t=0;m.c.forEach(function(a){a=PData.intersect(h,a.i);n.push(a);t+=a.length});var k=0,r;n.forEach(function(y){r=e.bars[a++];r.x=t?k*d/t:0;r.w=t?y.length*d/t:0;k+=y.length})});e.attsG.selectAll(".bar").transition().attr("x",function(a){return a.x}).attr("width",function(a){return a.w});PState.set(PSTATE_READY)}var e=this;
...
var h=[],q=0;k.forEach(function(a){0<a.i.length&&(h.push(a),q+=a.i.length)});var v=0;h.forEach(function(a){d.bars.push({x0:v*e/q,w0:a.i.length*e/q,x:0,w:0,y:y+18,c:a});v+=a.i.length});d.facets.push({i:g,l:r.def.l,y:y+14,c:h})});this.attsG.selectAll(".mbm-att-title").data(d.facets).enter().append("text").attr("class",function(a,d){return 0==d?"mbm-att-title obj-sel":"mbm-att-title"}).attr("x","0").attr("y",function(a){return a.y}).text(function(a){return a.l}).on("click",function(a,e){d.attSel!=e&&
...
" ("+h.length+")",id:m+"."+e+"."+f+"."+k})});k.z.push(q)}});d.trees.push(k)});var p=d3.layout.treemap().padding([14,3,3,3]).size([e,this.settings.h]).round(!0).children(function(a,d){return 2==d?null:a.z}).value(function(a){return a.i.length});d.treemaps=[];d.trees.forEach(function(a){d.treemaps.push(p.nodes(a))});d.renderTree(0)};VizMBMap.prototype.setSel=function(a){};
...
this.rCats.length]).rangeRound([0,e]),p=d3.scale.linear().domain([0,100]).range([g,0]);this.yScale=p;f=d3.scale.ordinal().rangeRoundBands([0,e]);f.domain(this.rCats.map(function(a){return a.l}));var f=d3.svg.axis().scale(f).orient("bottom"),n=d3.svg.axis().scale(p).orient("left").ticks(4);this.chart=e=d3.select(d.get(0)).append("svg").attr("width",e+D3FG_MARGINS.left+D3FG_MARGINS.right).attr("height",g+D3FG_MARGINS.top+D3FG_MARGINS.bottom).append("g").attr("transform","translate("+D3FG_MARGINS.left+
","+D3FG_MARGINS.top+")");e.append("g").attr("class","x axis").attr("transform","translate(0,"+g+")").call(f);e.append("g").attr("class","y axis").call(n);e.selectAll(".bar").data(this.rCats).enter().append("rect").attr("class","bar").attr("x",function(a,d){return m(d)+2}).attr("y",function(a){return p(100)}).attr("fill",function(a){return a.c}).attr("height",function(a){return g-p(100)}).attr("width",h-4);a.brush=d3.svg.brush().x(m).extent([0,this.rCats.length]).on("brushend",function(){if(d3.event.sourceEvent){var d=
a.brush.extent(),k=[Math.floor(d[0]),Math.floor(d[1])];k[0]>=k[1]&&(k[0]=Math.floor(d[0]),k[1]=Math.ceil(d[1]));k[1]=Math.max(k[1],k[0]+1);d3.select(this).transition().call(a.brush.extent(k));a.b0=k[0];a.b1=k[1]-1;a.min=a.rCats[a.b0].min;a.max=a.rCats[a.b1].max;a.isDirty(!0)}});a.brushg=e.append("g");a.brushg.attr("class","brush").call(a.brush);a.brushg.selectAll("rect").attr("y",-1).attr("height",g+4);a.brushg.selectAll(".resize").append("path").attr("d",function(a){var d=(a=+("e"==a))?1:-1,e=g/
4;return"M"+.5*d+","+e+"A6,6 0 0 "+a+" "+6.5*d+","+(e+6)+"V"+(2*e-6)+"A6,6 0 0 "+a+" "+.5*d+","+2*e+"ZM"+2.5*d+","+(e+8)+"V"+(2*e-8)+"M"+4.5*d+","+(e+8)+"V"+(2*e-8)})}};
...
f]);m.domain(this.rCats.map(function(a){return a.l}));var m=d3.svg.axis().scale(m).orient("bottom"),p=d3.svg.axis().scale(h).orient("left").ticks(4),n=this.insertPt(),t=_.template(document.getElementById("dltext-filter-date-ctrl").innerHTML);n.append(t({id:this.id}));n.find("input[name=dctrl-"+a.id+"]").change(function(){a.isDirty(!0)});this.chart=f=d3.select(n.get(0)).append("svg").attr("width",f+D3FG_MARGINS.left+D3FG_MARGINS.right).attr("height",d+D3FG_MARGINS.top+D3FG_MARGINS.bottom).append("g").attr("transform",
"translate("+D3FG_MARGINS.left+","+D3FG_MARGINS.top+")");f.append("g").attr("class","x axis").attr("transform","translate(0,"+d+")").call(m);f.append("g").attr("class","y axis").call(p);f.selectAll(".bar").data(this.rCats).enter().append("rect").attr("class","bar").attr("x",function(a,d){return g(d)+2}).attr("y",function(a){return h(100)}).attr("fill",function(a){return a.c}).attr("height",function(a){return d-h(100)}).attr("width",e-4);a.brush=d3.svg.brush().x(g).extent([0,this.rCats.length]).on("brushend",
function(){if(d3.event.sourceEvent){var d=a.brush.extent(),e=[Math.floor(d[0]),Math.floor(d[1])];e[0]>=e[1]&&(e[0]=Math.floor(d[0]),e[1]=Math.ceil(d[1]));e[1]=Math.max(e[1],e[0]+1);d3.select(this).transition().call(a.brush.extent(e));a.b0=e[0];a.b1=e[1]-1;a.min=a.rCats[a.b0].min;a.max=a.rCats[a.b1].max;a.isDirty(!0)}});a.brushg=f.append("g");a.brushg.attr("class","brush").call(a.brush);a.brushg.selectAll("rect").attr("y",-1).attr("height",d+4);a.brushg.selectAll(".resize").append("path").attr("d",
function(a){var e=(a=+("e"==a))?1:-1,f=d/4;return"M"+.5*e+","+f+"A6,6 0 0 "+a+" "+6.5*e+","+(f+6)+"V"+(2*f-6)+"A6,6 0 0 "+a+" "+.5*e+","+2*f+"ZM"+2.5*e+","+(f+8)+"V"+(2*f-8)+"M"+4.5*e+","+(f+8)+"V"+(2*f-8)})};PFilterDates.prototype.getState=function(){var a=jQuery("input[name=dctrl-"+this.id+"]:checked").val();return{e0:this.b0,e1:this.b1+1,c:a}};
...
y)throw Error("Transcript excerpt error: Cannot find timestamp "+widgetData.extract[0]);var q=a.indexOf(widgetData.extract[1]);if(-1==q)throw Error("Transcript excerpt error: Cannot find timestamp "+widgetData.extract[1]);a=a.substring(y-1,q-1)}if(a=d(a).split(/\r\n|\r|\n/g)){var l=jQuery("#xscript-tbl"),g=0,h,v=0,B=0,u="";_.each(a,function(a){a=d(a);1<a.length&&("["===a.charAt(0)&&"0"<=a.charAt(1)&&"9">=a.charAt(1)?(h=k(a),0<u.length&&(B&&r.push({s:v,e:h}),l.append('<div class="row"><div class="timecode" data-timecode="'+
v+'" data-tcindex="'+g++ +'">'+B+'</div><div class="xscript">'+u+"</div></div>"),u=""),B=a,v=h):(0<u.length&&(u+="<br/>"),u+=a))});0<u.length&&(r.push({s:v,e:324E5}),l.append('<div class="row"><div class="timecode" data-timecode="'+v+'" data-tcindex="'+g+'">'+B+'</div><div class="xscript">'+u+"</div></div>"));if("undefined"!=typeof n&&null!=n){var G=new XMLHttpRequest;G.onload=function(a){e(G.responseText,l)};G.open("GET",n,!0);G.send()}}}function r(a){var d,k=widgetData.tcIndex;_.find(widgetData.tcArray,
...
PData.getAttID(a);var k;"_"==a.def.l.charAt(0)?k="<div>"+e+"</div>":(k='<div><span class="att-label">'+a.def.l+":</span> ","I"==a.def.t&&(k+="<br/>"),k+=e+"</div>");m.append(k)}});p=null;z=0;widgetData.extract=null;widgetData.xscriptOn=!1;widgetData.playing=!1;if(prspdata.e.i.modal.scOn&&(p=prspdata.e.i.sc.atts[g])&&(e=J.a[p])){var h=!0;a();z=1;m.append('<iframe id="sc-widget" class="player" width="95%" height="150" src="http://w.soundcloud.com/player/?url='+e+'"></iframe></p>');var u=SC.Widget(document.getElementById("sc-widget"));
...
widgetData.widget.seekTo(a/1E3)}});n=null;(l=prspdata.e.i.t.t2Atts[g])&&""!==l&&"disable"!=l&&(n=J.a[l]);var v=new XMLHttpRequest;v.onload=function(a){f(v.responseText)};v.open("GET",e,!0);v.send()}}function g(a){a=N+a;-1==a?a=A.length-1:a==A.length&&(a=0);a!=N&&(N=a,q())}function v(a){g(-1)}function u(a){g(1)}var m=jQuery("#inspect-content"),p=null,z=0,n,A=null;l&&(A=l.getSel());if(null!=A&&0!=A.length){var t,J,N=0;q();jQuery("#btn-inspect-left").click(v);jQuery("#btn-inspect-right").click(u);var K=
...
(e.find(".osel").button("enable"),e.find(".osel").addClass("pulse"),e.find(".xsel").button("enable")):(e.find(".osel").button("disable"),e.find(".osel").removeClass("pulse"),e.find(".xsel").button("disable"))}function m(a){PState.set(PSTATE_UPDATE);l&&l.clearSel();h(!1);PState.set(PSTATE_READY);a.preventDefault()}function p(a){l&&l.doOptions();a.preventDefault()}function n(a){var d=jQuery("#dialog-vnotes").dialog({width:300,height:300,modal:!0,buttons:[{text:dlText.ok,click:function(){d.dialog("close")}}]});
...
f.c);break;case "F":u=new VizFlow(q,f.c);break;case "m":u=new VizMBMap(q,f.c)}v=k;var m=u.flags();m&V_FLAG_HSCRL?(g.find("div.viz-content").addClass("h-scroll"),g.find("div.viz-result").addClass("viz-fit-w"),g.find("div.viz-result").removeClass("viz-max-w")):(g.find("div.viz-content").removeClass("h-scroll"),g.find("div.viz-result").removeClass("viz-fit-w"),g.find("div.viz-result").addClass("viz-max-w"));m&V_FLAG_VSCRL?(g.find("div.viz-content").addClass("v-scroll"),g.find("div.viz-result").addClass("viz-fit-h"),
g.find("div.viz-result").removeClass("viz-max-h")):(g.find("div.viz-content").removeClass("v-scroll"),g.find("div.viz-result").removeClass("viz-fit-h"),g.find("div.viz-result").addClass("viz-max-h"));z=[];if(m&V_FLAG_LGND){g.find(".hslgnd").button("enable");var p=g.find("div.lgnd-container div.lgnd-scroll");p.empty();if(m&V_FLAG_SLGND){var n=u.getFeatureAtts(),G=PData.getAttID(n);p.append('<div class="lgnd-template" data-index="0"><div class="lgnd-title">'+G.def.l+'</div><div class="lgnd-entry lgnd-sh"><input type="checkbox" checked="checked" class="lgnd-entry-check"/><i>'+
dlText.sha+'</i></div><div class="lgnd-group"></div></div>');z.push(n);y(0,n)}else{var I=!1;prspdata.e.g.ts.forEach(function(a,d){var k=PData.getTmpltID(a),e=u.getLocAtts(d);if(e&&0<e.length||!(m&V_FLAG_LOC)){var f=u.getFeatureAtts(d);if(0<f.length){I&&p.append("<hr/>");var g=jQuery('<div class="lgnd-template" data-index="'+d+'"><div class="lgnd-title">'+k.l+"</div></div>");e&&e.forEach(function(a,d){var k=PData.getAttID(a);g.append('<div class="lgnd-entry lgnd-locate" data-id="'+a+'"><input type="checkbox" checked="checked" class="lgnd-entry-check"/><span class="lgnd-value-title">'+
k.def.l+"</span></div>")});var l='<select class="lgnd-select">';f.forEach(function(a,d){var k=PData.getAttID(a);l+='<option value="'+a+'">'+k.def.l+"</option>"});l+="</select>";k=jQuery(l);k.change(r);jQuery(g).append(k);jQuery(g).append('<div class="lgnd-entry lgnd-sh"><input type="checkbox" checked="checked" class="lgnd-entry-check"/><i>'+dlText.sha+'</i></div><div class="lgnd-group"></div>');p.append(g);f=f[0];z.push(f);y(d,f);I=!0}}})}g.find("div.lgnd-container").show()}else g.find(".hslgnd").button("disable"),
g.find("div.lgnd-container").hide();n=jQuery("#selector-v"+a);m&V_FLAG_SEL?(n.removeAttr("disabled"),n.prop("checked",!0)):(n.attr("disabled",!0),n.prop("checked",!1));m&V_FLAG_OPT?g.find(".vopts").button("enable"):g.find(".vopts").button("disable");(n=u.hint())||"string"==typeof f.n&&""!=f.n?(g.find(".vnote").button("enable"),n=n?"string"==typeof f.n&&""!=f.n?n+(".<br/>"+f.n):n+".":f.n,jQuery("#vnotes-txt").empty().append(n)):g.find(".vnote").button("disable");u.setup();h(!1);A&&e&&u.render(A);l=
u}var q={},v=0,l=null,z=[],A=null;q.getFrameID=d;q.getIndex=function(){return a};q.setViz=function(a,k){a!=v&&(jQuery(d()+" div.view-control-bar select.view-viz-select").val(a),u(a,k))};q.initDOM=function(r){var l=document.getElementById("dltext-viewframe-dom").innerHTML;jQuery("#viz-display-frame").append('<div id="view-frame-'+a+'">'+l+"</div>");var l=jQuery(d()),y=prspdata.bClrs.vf;y&&0<y.length&&l.find("div.view-control-bar").css("background-color",y);l.find("div.lgnd-container").draggable({handle:l.find("div.lgnd-handle"),
containment:"parent"});var q=l.find("div.view-control-bar select.view-viz-select");prspdata.e.vf.forEach(function(a,d){q.append('<option value="'+d+'">'+a.l+"</option>")});q.val(r);q.change(e);l.find("div.view-control-bar button:first").button({icons:{primary:"ui-icon-bookmark"},text:!1}).click(f).next().button({icons:{primary:"ui-icon-search"},text:!1}).click(g).next().button({icons:{primary:"ui-icon-cancel"},text:!1}).click(m).next().button({icons:{primary:"ui-icon-gear"},text:!1}).click(p).next().button({icons:{primary:"ui-icon-help"},
text:!1}).click(n).next();l.find("div.lgnd-container").click(k);u(r,!1)};q.getSelLocAtts=function(a){var k=[];jQuery(d()+' div.lgnd-container div.lgnd-scroll div.lgnd-template[data-index="'+a+'"] div.lgnd-locate input:checked').each(function(){var a=jQuery(this).parent().data("id");k.push(a)});return k};q.getSelFeatAtts=function(a){var k=[],e,f;jQuery(d()+' div.lgnd-container div.lgnd-scroll div.lgnd-template[data-index="'+a+'"] div.lgnd-group div.lgnd-value input:checked').each(function(){e=jQuery(this).parent().data("index");
...
q.showStream=function(a){A=a;l&&l.render(a)};q.setStream=function(a){A=a};q.selBtns=function(a){h(a)};q.clearSel=function(){l&&l.clearSel();h(!1)};q.setSel=function(a){return l&&l.flags()&V_FLAG_SEL?(l.setSel(a),h(0<a.length),!0):!1};q.resize=function(){l&&l.resize()};q.title=function(){return PData.getVizIndex(v).l};q.flushLgnd=function(){jQuery(d()).find("div.lgnd-container").css("left","10px")};return q}
var PData=function(){function a(a,e,f){jQuery.ajax({type:"POST",url:prspdata.ajax_url,data:{action:"prsp_get_records",tmplt_id:prspdata.t[a].id,from:e,count:f},success:function(e,r,g){r=p[a];e=JSON.parse(e);r.d=r.d?r.d.concat(e):e;r.n+=f;d()},error:function(a,d,k){alert(k)}})}function d(){for(var d=!0,f=0;f<prspdata.t.length;f++){var g=p[f].n,h=prspdata.t[f].n;if(g<h){d=!1;h-=g;a(f,g,h<e?h:e);break}}d&&(t=!0,setTimeout(function(){jQuery("body").trigger("prospect",{pstate:PSTATE_PROCESS,component:0})},
500))}var e=1E3,f,g,h,m=[31,28,31,30,31,30,31,31,30,31,30,31],p=[],n=0,t=!1;return{init:function(){document.getElementById("dltext-from");f=document.getElementById("dltext-to").innerHTML;g=document.getElementById("dltext-approximately").innerHTML;h=document.getElementById("dltext-now").innerHTML;"number"==typeof prspdata.chunk&&0<prspdata.chunk&&(e=prspdata.chunk);prspdata.t.forEach(function(a){p.push({i:n,n:0,d:null});n+=a.n});d()},intersect:function(a,d){for(var e=[],f=a.length,g=d.length,h,l,m=
0,p=0;m<f&&p<g;)h=a[m],l=d[p],h>l?p++:l>h?m++:(e.push(h),m++,p++);return e},union:function(a,d){for(var e=[],f=0,g=0,h,l;f<a.length&&g<d.length;)h=a[f],l=d[g],h<l?(e.push(h),f++):(h==l?(e.push(h),f++):e.push(l),g++);for(;f<a.length;f++)e.push(a[f]);for(;g<d.length;g++)e.push(d[g]);return e},strcmp:function(a,d){for(var e=0,f=Math.max(a.length,d.length);e<f&&a.charAt(e)===d.charAt(e);++e);return e===f?0:a.charAt(e)>d.charAt(e)?-1:1},newIndexStream:function(a){var d={};d.s=new Uint16Array(n);d.t=[];
d.l=0;if(a){for(a=0;a<n;a++)d.s[a]=a;for(a=0;a<p.length;a++){var e=p[a];d.t.push({i:e.i,n:e.n})}d.l=n}return d},aIndex2Tmplt:function(a){for(var d=0;d<p.length;d++){var e=p[d];if(e.i<=a&&a<e.i+e.n)return d}},attInTmplt:function(a,d){return-1!=prspdata.t[d].def.a.findIndex(function(d){return d==a})},stream1stTEntry:function(a,d){var e=a.t[d];return 0==e.n?-1:e.i},getRecByIndex:function(a){for(var d=0;d<p.length;d++){var e=p[d];if(0<e.n&&e.i<=a&&a<e.i+e.n)return e.d[a-e.i]}return null},procAttTxt:function(a,
...
prspdata.site_url+"?p="+a.wp+'" target="_blank">'+a.l+"</a> ")});return u}}return null},getRecAtt:function(a,d,e){for(var f=0;f<p.length;f++){var g=p[f];if(0<g.n&&g.i<=a&&a<g.i+g.n){a=g.d[a-g.i].a[d];if(null==a||"undefined"==typeof a)break;return e?a:PData.procAttTxt(d,a)}}return null},getAbsIByID:function(a){for(var d=0;d<p.length;d++){var e=p[d];if(0<e.n)for(var f=0,g=e.n-1,h,l;f<=g;)if(h=f+g>>1,l=e.d[h],l=PData.strcmp(a,l.id),0>l)f=h+1;else if(0<l)g=h-1;else return e.i+h}return null},getRecByID:function(a){for(var d=
0;d<p.length;d++){var e=p[d];if(0<e.n)for(var f=0,g=e.n-1,h,l,m;f<=g;)if(h=f+g>>1,m=e.d[h],l=PData.strcmp(a,m.id),0>l)f=h+1;else if(0<l)g=h-1;else return m}return null},getAttID:function(a){for(var d=0,e=prspdata.a.length-1,f,g;d<=e;)if(f=d+e>>1,g=PData.strcmp(a,prspdata.a[f].id),0>g)d=f+1;else if(0<g)e=f-1;else return prspdata.a[f];return null},getAttIndex:function(a){return prspdata.a[a]},getNumETmplts:function(){return prspdata.e.g.ts.length},getETmpltIndex:function(a){return prspdata.e.g.ts[a]},
getTmpltID:function(a){for(var d=0;d<prspdata.t.length;d++)if(a==prspdata.t[d].id)return prspdata.t[d].def},getAttLgndRecs:function(a,d,e,f){var g,h=e.length,l;switch(d.def.t){case "V":var m=function(a){for(var f=0;f<h;f++)if(g=e[f],"number"===typeof g){if(l=d.l[g],l.l==a)return l}else{l=d.l[g[0]];var k=l.z[g[1]];if(k.l==a)return k.v&&""!=k.v?k:l}return null};if(f&&""!=d.def.d){var p=[],n;a.forEach(function(a){n=m(a);null!=n&&p.push(n)});return 0<p.length?p:null}if(""!=d.def.d){for(f=0;f<a.length;f++)if(n=
m(a[f]),null!=n)return n;break}else return m(a[0]);case "T":for(n=0;n<h;n++)if(g=e[n],l=d.l[g],-1!=a.indexOf(l.d))return l;break;case "N":for(n=0;n<h;n++)if(g=e[n],l=d.l[g],"undefined"!=typeof l.d.min){if(l.d.min<=a)if("undefined"!=typeof l.d.max){if(a<=l.d.max)return l}else return l}else if(a<=l.d.max)return l;break;case "D":for(n=0;n<h;n++)if(g=e[n],l=d.l[g],"undefined"!=typeof l.d.max.y){if("undefined"!=typeof a.max&&"open"!=a.max){if(a.max.y<l.d.min.y)continue;if(a.max.y==l.d.min.y&&a.max.m&&
...
d||PData.getAttLgndRecs([f.l],a,d,!1))&&e.push({l:f.l,c:f.v,i:[]})})}),e;case "N":return a.l.forEach(function(f){(null==d||PData.getAttLgndRecs(f.d.min,a,d,!1))&&e.push({l:f.l,c:f.v,min:f.d.min,max:f.d.max,i:[]})}),e;case "D":var f,g;a.l.forEach(function(h){f=PData.objDate(h.min,1,!1);if(null==d||PData.getAttLgndRecs(f,a,d,!1))g=null==h.max.y?TODAY:PData.objDate(h.max,12,!1),e.push({l:h.l,c:h.v,min:f,max:g,i:[]})});return e}},getRCats:function(a,d){var e=[];switch(a.def.t){case "T":return a.l.forEach(function(a){d?
e.push({l:a.l,x:a.d,c:a.v,i:[]}):e.push({l:a.l,x:a.d,c:a.v})}),e;case "V":return a.l.forEach(function(a){d?e.push({l:a.l,c:a.v,i:[]}):e.push({l:a.l,c:a.v});a.z.forEach(function(a){d?e.push({l:a.l,c:a.v,i:[]}):e.push({l:a.l,c:a.v})})}),e;case "N":if("undefined"==typeof a.r.min||"undefined"==typeof a.r.max)return null;var f=Math.pow(10,a.r.g),g=a.r.min,h=0,l,m,n;for(0<a.l.length&&(l=a.l[0]);g<=a.r.max;){for(;h<a.l.length&&"undefined"!=typeof l.d.max&&g>l.d.max;)l=a.l[++h];n=0==a.l.length||g<l.d.min?
"#777777":h==a.l.length?"#777777":"undefined"==typeof l.d.max||g<=l.d.max?l.v:"#777777";m=g;g+=f;max=g-1;var p=m.toString();1<f&&4>p.length&&(p+="-"+max.toString());d?e.push({l:p,c:n,min:m,max:max,i:[]}):e.push({l:p,c:n,min:m,max:max})}return e;case "D":g=function(a){var d,e;if("undefined"==typeof a.y)return TODAY;d=a.y;"undefined"==typeof a.m?(e=12,a=31):(e=a.m,a="undefined"==typeof a.d?PData.lenMnth(d,e):a.d);return PData.date3Nums(d,e,a,!1)};m=function(a,d,e,f){"undefined"!=typeof f.m&&(d=f.m,
"undefined"!=typeof f.d&&(e=f.d));return PData.date3Nums(a,d,e,!1)};n=g(a.r.max);var f=a.r.g,p=a.r.min.y,t=1,w=1;"undefined"!=typeof a.r.min.m&&(t=a.r.min.m,"undefined"!=typeof a.r.min.d&&(w=a.r.min.d));var x=PData.date3Nums(p,t,w,!1),h=0,C,D;0<a.l.length&&(l=a.l[0],C=m(l.d.min.y,1,1,l.d.min),D=g(l.d.max));for(;x<=n;){var F={};d&&(F.i=[]);switch(f){case "d":F.l=p.toString()+"-"+t.toString()+"-"+w.toString();break;case "m":F.l=p.toString()+"-"+t.toString();break;case "y":case "t":case "c":F.l=p.toString()}for(;h<
a.l.length&&x>D;)l=a.l[++h],h<a.l.length&&(C=m(l.d.min.y,1,1,l.d.min),D=g(l.d.max));F.c=0==a.l.length||x<C?"#777777":h==a.l.length?"#777777":x<=D?l.v:"#777777";F.min=x;switch(f){case "d":++w>PData.lenMnth(p,t)&&(w=1,12<++t&&(t=1,p++));break;case "m":12<++t&&(t=1,p++);break;case "y":p++;break;case "t":p+=10;break;case "c":p+=100}F.max=PData.date3Nums(p,t,w,!1);e.push(F);x=PData.date3Nums(p,t,w,!1)}return e}},fillCats:function(a,d,e,f,g){var h=PData.getNumETmplts(),l=0,m,p=0,n,t,w,x,C=PData.getAttID(d);
for(m=f.t[0];p<f.l;){for(;0==m.n||m.i+m.n==p||!PData.attInTmplt(d,l)||e&&!PData.attInTmplt(e,l);){if(++l==h)return;m=f.t[l];p=m.i}n=f.s[p];t=PData.getRecByIndex(n);t=t.a[d];if("undefined"!=typeof t)switch(C.def.t){case "T":if(g){for(w=0;w<a.length;w++)if(x=a[w],t==x.l){x.i.push(n);break}w==a.length&&a.push({l:t,i:[n]})}else for(w=0;w<a.length;w++)if(x=a[w],-1!=t.indexOf(x.x)){x.i.push(n);break}break;case "V":t.forEach(function(d){for(w=0;w<a.length;w++)if(x=a[w],d==x.l){x.i.push(n);break}});break;
case "N":for(w=0;w<a.length;w++){x=a[w];if(t<x.min)break;if(x.min<=t&&t<=x.max){x.i.push(n);break}}break;case "D":for(t=PData.objDate(t.min,1,!1),w=0;w<a.length;w++){x=a[w];if(t<x.min)break;if(x.min<=t&&t<x.max){x.i.push(n);break}}}p++}g&&a.sort(function(a,d){return PData.strcmp(d.l,a.l)})},sortCats:function(a,d,e){var f=d.id,g,h,l;a.forEach(function(a){g=PData.getRecByIndex(a);datum=g.a[f];if("undefined"!=typeof datum)switch(d.def.t){case "T":for(h=0;h<e.length;h++)if(l=e[h],-1!=datum.indexOf(l.x)){l.i.push(a);
break}break;case "V":datum.forEach(function(d){for(h=0;h<e.length;h++)if(l=e[h],d==l.l){l.i.push(a);break}});break;case "N":for(h=0;h<e.length;h++){l=e[h];if(datum<l.min)break;if(l.min<=datum&&datum<=l.max){l.i.push(a);break}}break;case "D":var k=PData.objDate(datum.min,1,!1);for(h=0;h<e.length;h++){l=e[h];if(k<l.min)break;if(l.min<=k&&k<l.max){l.i.push(a);break}}}})},orderTBy:function(a,d,e){function f(a){return a}function g(a){return a[0]}function h(a){var d=1,e=1;"undefined"!=typeof a.min.m&&(d=
a.min.m,"undefined"!=typeof a.min.d&&(e=a.min.d));return PData.date3Nums(a.min.y,d,e,!1)}var l,m;switch(a.def.t){case "T":l=f;m="~";break;case "V":l=g;m="~";break;case "N":l=f;m=a.r.max;break;case "D":l=h,m=TODAY}var n=[],t=d.t[e];e=p[e];for(var E=0,w,x;E<t.n;)w=d.s[t.i+E++],x=e.d[w-e.i],x=x.a[a.id],"undefined"==typeof x?n.push({i:w,v:m}):n.push({i:w,v:l(x)});switch(a.def.t){case "T":case "V":n.sort(function(a,d){return PData.strcmp(d.v,a.v)});break;case "D":case "N":n.sort(function(a,d){return a.v-
d.v})}return n},getVizIndex:function(a){return prspdata.e.vf[a]},ready:function(){return t}}}();
jQuery(document).ready(function(a){function d(){var a;PState.set(PSTATE_PROCESS);null==w&&(w=PData.newIndexStream(!0));x=w;var d=!1,e,f;for(e=0;e<z.length;e++)if(f=z[e],a=jQuery('div.filter-instance[data-id="'+f.id+'"]'),d||f.f.isDirty(null)){d=!0;f=f.f;f.evalPrep();for(var g=PData.newIndexStream(!1),h=0,k,m,n=0,p=x.t[0],q=0,r=0,t=a.find(".apply-tmplt-0").is(":checked");h<x.l;){for(;0==p.n||p.i+p.n==h;)g.t.push({i:g.l-q,n:q}),q=0,p=x.t[++n],t=a.find(".apply-tmplt-"+n).is(":checked");k=x.s[h++];t?
(m=PData.getRecByIndex(k),f.eval(m)&&(g.s[g.l++]=k,q++),r++):(g.s[g.l++]=k,q++)}for(;n++<PData.getNumETmplts();)g.t.push({i:g.l-q,n:q}),q=0;f.isDirty(!1);f.out=g;f.evalDone(r);x=g}else x=f.f.out;PState.set(PSTATE_BUILD);v.showStream(x);l&&l.showStream(x);jQuery("#btn-recompute").removeClass("pulse")}function e(a){var d=jQuery("#annote");d.text(a);0<a.length?(jQuery("#btn-annote").button("enable"),d.show()):(jQuery("#btn-annote").button("disable"),d.hide())}function f(a){var d=_.find(prspdata.p,function(d){return a==
d.id});return d?d:null==C||0==D.length?null:(d=_.find(D,function(d){return a==d.id}))?d:null}function g(a){var d=jQuery("input[name=save-prspctv-dest]:checked").val();if(""==d)return null;var e=jQuery("#save-psrctv-note").val(),e=e.replace(/"/,""),f=jQuery("#save-psrctv-lbl").val().trim(),f=f.replace(/"/,""),g=!jQuery("#selector-v0").prop("disabled")&&jQuery("#selector-v0").is(":checked"),h=!jQuery("#selector-v1").prop("disabled")&&jQuery("#selector-v1").is(":checked"),k={f:[],s:null,a0:g,a1:h,v0:{l:v.title(),
s:v.getState()},v1:null};z.forEach(function(a){for(var d=[],e=jQuery('div.filter-instance[data-id="'+a.id+'"]'),f=0;f<PData.getNumETmplts();f++)d.push(e.find(".apply-tmplt-"+f).is(":checked"));k.f.push({id:a.attID,a:d,s:a.f.getState()})});A&&(k.s={id:B,s:A.getState()});l&&(k.v1={l:l.title(),s:l.getState()});var m={id:a,l:f,n:e,s:k};"local"==d?(D.push(m),C.setItem(prspdata.e.id,JSON.stringify(D))):"server"==d&&jQuery.ajax({type:"POST",url:prspdata.ajax_url,data:{action:"prsp_save_prspctv",id:a,l:f,
...
d[e]);l.click(n)}a.find("button.btn-filter-toggle").button({text:!1,icons:{primary:"ui-icon-carat-2-n-s"}}).click(p);a.find("button.btn-filter-del").button({text:!1,icons:{primary:"ui-icon-trash"}}).click(t);jQuery("#btn-recompute").addClass("pulse")}g.setup();return g}function y(){PState.set(PSTATE_PROCESS);var a=[],d=!1;if(null!=A&&null!=x){A.evalPrep();for(var e=0,f,g;e<x.l;)f=x.s[e++],g=PData.getRecByIndex(f),A.eval(g)&&a.push(f);A.isDirty(!1);A.evalDone(x.l)}PState.set(PSTATE_UPDATE);jQuery("#selector-v0").is(":checked")&&
...
a.s.f.length?"disable":"enable");k();null!=a.s.s&&(r(a.s.s.id,null,!0),A.setState(a.s.s.s),jQuery("#btn-toggle-selector").button("enable"),jQuery("#btn-apply-selector").button("enable"));jQuery("#selector-instance").hide();var h,m=!1;PState.set(PSTATE_BUILD);h=g(a.s.v0.l);v?(v.setViz(h,!1),v.selBtns(!1)):(v=PViewFrame(0),v.initDOM(h));null!=a.s.v1?(h=g(a.s.v1.l),l?(l.selBtns(!1),l.setViz(h,!1),l.setState(a.s.v1.s)):(v.flushLgnd(),l=PViewFrame(1),l.initDOM(h),l.setState(a.s.v1.s),m=!0)):l&&(l=null,
jQuery("#view-frame-1").remove(),jQuery("#selector-v1").prop("disabled",!0),m=!0);m&&v.resize();v.setState(a.s.v0.s);e(a.n);jQuery("#selector-v0").prop("checked",a.s.a0);jQuery("#selector-v1").prop("checked",a.s.a1);PData.ready()&&w&&(d(),A&&y());return!0}function q(a,d){var e=prspdata.bClrs[a];e&&0<e.length&&jQuery(d).css("background-color",e)}var v,l,z=[],A=null,B,E,w,x,C=null,D=[];jQuery("body").addClass("waiting");q("cb","#command-bar");q("fs","#filter-frame");q("sf","#selector-frame");PState.init();
