...
function PViewFrame(t){function u(b){b!==v&&(v=b,jQuery(q()+" div.lgnd-container div.lgnd-handle button.lgnd-update").prop("disabled",!b))}function q(){return"#view-frame-"+t}function M(b){b=jQuery(q()+" div.view-controls select.view-viz-select option:selected").val();PState.set(PSTATE_BUILD);G(b,!0);PState.set(PSTATE_READY)}function N(b){k.flags()&V_FLAG_LGND&&jQuery(q()+" div.lgnd-container").toggle("slide",{direction:"left"});b.preventDefault()}function O(b){function g(a){return a.replace(/^[ \f\t\v\u200b]+|[ \f\t\v\u200b]+$/g,
...
d+'"]').next().after('<div class="xscript">'+a+"</div>")})}function m(a){widgetData.tcArray=[];widgetData.tcIndex=-1;var d=widgetData.tcArray;a=new String(a);if(a=g(a).split(/\r\n|\r|\n/g)){var e=jQuery("#xscript-tbl"),p=0,b,h=0,n=0,r="";_.each(a,function(a){a=g(a);1<a.length&&("["===a.charAt(0)&&"0"<=a.charAt(1)&&"9">=a.charAt(1)?(b=c(a),0<r.length&&(n&&d.push({s:h,e:b}),e.append('<div class="row"><div class="timecode" data-timecode="'+h+'" data-tcindex="'+p++ +'">'+n+'</div><div class="xscript">'+
r+"</div></div>"),r=""),n=a,h=b):(0<r.length&&(r+="<br/>"),r+=a))});0<r.length&&(d.push({s:h,e:324E5}),e.append('<div class="row"><div class="timecode" data-timecode="'+h+'" data-tcindex="'+p+'">'+n+'</div><div class="xscript">'+r+"</div></div>"));"undefined"!==typeof t&&null!=t&&jQuery.ajax({type:"POST",url:prspdata.ajax_url,data:{action:"prsp_get_transcript",transcript:t,excerpt:widgetData.extract},success:function(a,d,b){f(JSON.parse(a),e)},error:function(a,d,e){alert(e)}})}}function q(a){var d,
...
b.preventDefault()}function K(b,g){jQuery(q()+' div.lgnd-container div.lgnd-scroll div.lgnd-template[data-index="'+b+'"] div.lgnd-group input.lgnd-entry-check').prop("checked",g);u(!0)}function L(b){var g=jQuery(b.target).closest("div.lgnd-template").data("index"),c=b.target.className;switch(c){case "lgnd-update":k&&x&&(PState.set(PSTATE_BUILD),B(!1),k.render(x),u(!1),PState.set(PSTATE_READY));break;case "lgnd-entry-check":c=jQuery(b.target).closest("div.lgnd-entry");b=jQuery(b.target).is(":checked");
c.hasClass("lgnd-sh")?K(g,b):c.hasClass("lgnd-locate")?(c.data("id"),u(!0)):c.hasClass("lgnd-value")&&(c.data("index"),u(!0));break;case "lgnd-viz":case "lgnd-value-title":c=jQuery(b.target).closest("div.lgnd-entry");c.hasClass("lgnd-locate")?(b=c.data("id"),jQuery(q()+' div.lgnd-container div.lgnd-scroll div.lgnd-template[data-index="'+g+'"] div.lgnd-locate input.lgnd-entry-check').prop("checked",!1),jQuery(q()+' div.lgnd-container div.lgnd-scroll div.lgnd-template[data-index="'+g+'"] div.lgnd-locate[data-id="'+
b+'"] input.lgnd-entry-check').prop("checked",!0),u(!0)):c.hasClass("lgnd-value")&&(b=c.data("index"),jQuery(q()+' div.lgnd-container div.lgnd-scroll div.lgnd-template[data-index="'+g+'"] div.lgnd-group input.lgnd-entry-check').prop("checked",!1),jQuery(q()+' div.lgnd-container div.lgnd-scroll div.lgnd-template[data-index="'+g+'"] div.lgnd-group div.lgnd-value[data-index="'+b+'"] input.lgnd-entry-check').prop("checked",!0),u(!0));break;case "lgnd-template":case "lgnd-select":case "":break;default:c.match(/lgnd-sh/i)&&
(c=jQuery(b.target).find("input.lgnd-entry-check"),b=!c.is(":checked"),c.prop("checked",b),K(g,b))}}function S(b){var g=jQuery(b.target).closest("div.lgnd-template").data("index");b=jQuery(b.target).val();F(g,b);u(!0)}function F(b,g){var c,f=jQuery(q()+' div.lgnd-container div.lgnd-scroll div.lgnd-template[data-index="'+b+'"] div.lgnd-group');f.empty();w[b]=g;var k=PData.aByID(g);"undefined"!==typeof k.r.u&&(c='<div class="lgnd-value lgnd-entry" data-index="-1"><input type="checkbox" checked="checked" class="lgnd-entry-check"/><div class="lgnd-viz" style="background-color: '+
...
c=b.v&&""!==b.v?c+('<div class="lgnd-viz" style="background-color: '+b.v+'"></div>'):c+'<div class="lgnd-viz lgnd-viz-empty"></div>';c+=' <span class="lgnd-value-title">&raquo; '+b.l+"</span></div>";f.append(c)})});jQuery(q()+' div.lgnd-container div.lgnd-scroll div.lgnd-template[data-index="'+b+'"] div.lgnd-sh input').prop("checked",!0)}function G(b,g){var c=PData.vByN(b);k&&(k.teardown(),k=null);var l=jQuery(q());l.find("div.viz-content div.viz-result").empty();var m;switch(c.vf){case "M":m=new VizMap(f,
c.c);break;case "p":m=new VizMap2(f,c.c);break;case "C":m=new VizCards(f,c.c);break;case "P":m=new VizPinboard(f,c.c);break;case "T":m=new VizTime(f,c.c);break;case "D":m=new VizDirectory(f,c.c);break;case "t":m=new VizTextStream(f,c.c);break;case "S":m=new VizStackChart(f,c.c);break;case "N":m=new VizNetWheel(f,c.c);break;case "F":m=new VizFlow(f,c.c);break;case "B":m=new VizBrowser(f,c.c);break;case "m":m=new VizMBMap(f,c.c)}I=b;var v=m.flags();v&V_FLAG_HSCRL?(l.find("div.viz-content").addClass("h-scroll"),
l.find("div.viz-result").addClass("viz-fit-w"),l.find("div.viz-result").removeClass("viz-max-w")):(l.find("div.viz-content").removeClass("h-scroll"),l.find("div.viz-result").removeClass("viz-fit-w"),l.find("div.viz-result").addClass("viz-max-w"));v&V_FLAG_VSCRL?(l.find("div.viz-content").addClass("v-scroll"),l.find("div.viz-result").addClass("viz-fit-h"),l.find("div.viz-result").removeClass("viz-max-h")):(l.find("div.viz-content").removeClass("v-scroll"),l.find("div.viz-result").removeClass("viz-fit-h"),
l.find("div.viz-result").addClass("viz-max-h"));w=[];if(v&V_FLAG_LGND){l.find(".hslgnd").button("enable");var a=l.find("div.lgnd-container div.lgnd-scroll");a.empty();if(v&V_FLAG_SLGND){var p=m.getFeatureAtts(),d=PData.aByID(p);a.append('<div class="lgnd-template" data-index="0"><div class="lgnd-title">'+d.def.l+'</div><div class="lgnd-entry lgnd-sh"><input type="checkbox" checked="checked" class="lgnd-entry-check"/><i>'+dlText.sha+'</i></div><div class="lgnd-group"></div></div>');w.push(p);F(0,p)}else{var e=
!1;prspdata.e.g.ts.forEach(function(d,b){var p=PData.tByID(d),c=m.getLocAtts(b);if(c&&0<c.length||!(v&V_FLAG_LOC)){var g=m.getFeatureAtts(b);if(0<g.length){e&&a.append("<hr/>");var f=jQuery('<div class="lgnd-template" data-index="'+b+'"><div class="lgnd-title">'+p.l+"</div></div>");c&&c.forEach(function(a,d){var e=PData.aByID(a);f.append('<div class="lgnd-entry lgnd-locate" data-id="'+a+'"><input type="checkbox" checked="checked" class="lgnd-entry-check"/><span class="lgnd-value-title">'+e.def.l+
"</span></div>")});var k='<select class="lgnd-select">';g.forEach(function(a,d){var e=PData.aByID(a);k+='<option value="'+a+'">'+e.def.l+"</option>"});k+="</select>";p=jQuery(k);p.change(S);jQuery(f).append(p);jQuery(f).append('<div class="lgnd-entry lgnd-sh"><input type="checkbox" checked="checked" class="lgnd-entry-check"/><i>'+dlText.sha+'</i></div><div class="lgnd-group"></div>');a.append(f);g=g[0];w.push(g);F(b,g);e=!0}}})}l.find("div.lgnd-container").show()}else l.find("button.hslgnd").button("disable"),
l.find("div.lgnd-container").hide();u(!1);v&V_FLAG_SEL?(l.find(".hilite").button("enable"),jQuery("#save-prspctv-h"+t).prop("disabled",!1).prop("checked",!1)):(l.find(".hilite").button("disable"),jQuery("#save-prspctv-h"+t).prop("disabled",!0).prop("checked",!1));v&V_FLAG_OPT?l.find(".vopts").button("enable"):l.find(".vopts").button("disable");(p=m.hint())||"string"===typeof c.n&&""!==c.n?(l.find(".vnote").button("enable"),p=p?"string"===typeof c.n&&""!==c.n?p+(".<br/>"+c.n):p+".":c.n,jQuery("#vnotes-txt").empty().append(p)):
l.find(".vnote").button("disable");m.setup();B(!1);x&&g&&m.render(x);k=m}var f={},I=0,k=null,w=[],v=null,x=null;f.getFrameID=q;f.getIndex=function(){return t};f.setViz=function(b,g){b!=I&&(jQuery(q()+" div.view-controls select.view-viz-select").val(b),G(b,g))};f.initDOM=function(b){var g=document.getElementById("dltext-view-controls").innerHTML;jQuery("#viz-frame").append('<div id="view-frame-'+t+'">'+g+"</div>");var g=jQuery(q()),c=prspdata.bClrs.vf;c&&0<c.length&&g.find("div.view-controls").css("background-color",
...
a.id+'"]'),d=0;d<PData.eTNum();d++)b.push(e.find(".apply-tmplt-"+d).is(":checked"));h.f.push({id:a.attID,a:b,s:a.f.getState()})});for(var n=0;2>n;n++){var r=w[n];null!==r&&jQuery("#save-prspctv-h"+n).is(":checked")&&(h["h"+n]={id:v[n],s:r.getState()})}var E={id:a,l:b,n:e,s:h};"local"==d?(c.push(E),g.setItem(prspdata.e.id,JSON.stringify(c))):"server"==d&&jQuery.ajax({type:"POST",url:prspdata.ajax_url,data:{action:"prsp_save_prspctv",id:a,l:b,x:prspdata.e.id,n:e,s:JSON.stringify(h)},success:function(a,
b,e){"0"!=a&&prspdata.p.push(E)},error:function(a,b,e){alert(e)}});return d}function O(){var a,b=[],d=!1;(function(){var a=jQuery("#prspctv-mlist");a.empty();c.forEach(function(b){a.append('<li data-type="l" data-id="'+b.id+'"><span class="label">'+b.l+'</span> <button class="del">'+dlText.del+'</button> <button class="edit">'+dlText.edit+"</button></li>")});for(var d=0;d<g.length;d++){var n=g.key(d);if(n!=prspdata.e.id){var f=g.getItem(n);b.push({id:n,ps:JSON.parse(f)})}}b.forEach(function(b,d){b.ps.forEach(function(h){a.append('<li data-type="x" data-xid="'+
b.id+'" data-xindex="'+d+'" data-id="'+h.id+'"><i class="label">'+h.l+'</i> <button class="del">'+dlText.del+'</button> <button class="edit">'+dlText.edit+"</button></li>")})})})();jQuery("#prspctv-mlist").click(function(a){if("BUTTON"==a.target.nodeName){var h=jQuery(a.target).hasClass("del"),f=jQuery(a.target).parent(),k=f.data("type"),m=f.data("id");if(h){switch(k){case "l":a=c.findIndex(function(a){return m==a.id});-1!=a&&(c.splice(a,1),0==c.length?g.removeItem(prspdata.e.id):g.setItem(prspdata.e.id,
JSON.stringify(c)));break;case "x":a=f.data("xindex"),h=b[a],a=h.ps.findIndex(function(a){return m==a.id}),-1!=a&&(h.ps.splice(a,1),d=!0)}f.remove()}else{var l;switch(k){case "l":l=_.find(c,function(a){return m==a.id});break;case "x":a=f.data("xindex"),h=b[a],l=_.find(h.ps,function(a){return m==a.id})}jQuery("#edit-prspctv-lbl").val(l.l);jQuery("#edit-prspctv-note").val(l.n);var q=jQuery("#dialog-edit-prsrctv").dialog({width:340,height:270,modal:!0,buttons:[{text:dlText.ok,click:function(){l.l=jQuery("#edit-prspctv-lbl").val();
l.n=jQuery("#edit-prspctv-note").val();f.find(".label").text(l.l);"x"==k?d=!0:g.setItem(prspdata.e.id,JSON.stringify(c));q.dialog("close")}},{text:dlText.cancel,click:function(){q.dialog("close")}}]})}}});a=jQuery("#dialog-manage-prsrctv").dialog({width:450,height:350,modal:!0,buttons:[{text:dlText.ok,click:function(){d&&b.forEach(function(a){0<a.ps.length?g.setItem(a.id,JSON.stringify(a.ps)):g.removeItem(a.id)});jQuery("#prspctv-mlist").off("click");a.dialog("close")}}]})}function B(a){window.location.href=
...
a)h=new PFilterRemove(e),c={t:[!0,!0,!0,!0]};else switch(c=PData.aByID(a),c.def.t){case "V":h=new PFilterVocab(e,c);break;case "T":h=new PFilterText(e,c);break;case "g":h=new PFilterTags(e,c);break;case "N":h=new PFilterNum(e,c);break;case "D":h=new PFilterDates(e,c)}if(null!==d)b=jQuery("#dialog-hilite-"+d+" span.filter-id").html(c.def.l),b=jQuery("#hilite-"+d),b.empty();else{k.push({id:e,attID:a,f:h,out:null});d=_.template(document.getElementById("dltext-filter-head").innerHTML);jQuery("#filter-instances").append(d({newID:e,
title:h.title(),apply:I}));d=jQuery('div.filter-instance[data-id="'+e+'"]');for(a=0;a<PData.eTNum();a++){var f=d.find(".apply-tmplt-"+a);f.prop("disabled",!c.t[a]);f.prop("checked",b[a]&&c.t[a]);f.click(Q)}d.find("button.btn-filter-toggle").button({text:!1,icons:{primary:"ui-icon-carat-2-n-s"}}).click(P);d.find("button.btn-filter-del").button({text:!1,icons:{primary:"ui-icon-trash"}}).click(R);jQuery("#btn-f-state").prop("disabled",!1).html(dlText.dofilters)}h.setup();return h}function K(a,b,d,e){jQuery("#filter-list li").removeClass("selected");
var c,f,g,k;jQuery("#filter-list li").each(function(b){c=jQuery(this);f=c.data("id");"_remove"==f?a?c.show():c.hide():d?(g=PData.aByID(f),k=!1,g.t.forEach(function(a,b){k=k||a&&d[b]}),k?c.show():c.hide()):c.show()});var l,m={height:300,width:350,modal:!0,buttons:[{text:dlText.add,click:function(){var a=jQuery("#filter-list li.selected");1===a.length&&e(a.data("id"));l.dialog("close")}},{text:dlText.cancel,click:function(){l.dialog("close")}}]};b&&(m.appendTo="#dialog-2");l=jQuery("#dialog-choose-att").dialog(m)}
function L(a){PState.set(PSTATE_PROCESS);var c=f[a],d=c.getBMData(),e=[];a=w[a];if(null!==b){a.evalPrep();var h=0,g,k,l=0,m=b.t[0];a:for(;h<b.l;){for(;!d.t[l]||0==m.n||m.i+m.n==h;){if(++l===PData.eTNum())break a;m=b.t[l];h=m.i}g=b.s[h++];d.r[g>>4]&1<<(g&15)&&(k=PData.rByN(g),a.eval(k)&&e.push(g))}a.evalDone(b.l)}PState.set(PSTATE_UPDATE);0<e.length?c.setSel(e):c.clearSel();PState.set(PSTATE_READY)}function S(a,b){var d;d=jQuery("#dialog-hilite-"+a).dialog({height:275,width:Math.min(jQuery(window).width()-
20,675),modal:!0,appendTo:"#dialog-1",buttons:[{text:dlText.chsatt,click:function(){K(!1,!0,b,function(b){v[a]=b;w[a]=H(b,null,a)})}},{text:dlText.ok,click:function(){d.dialog("close");null!==w[a]&&L(a)}},{text:dlText.cancel,click:function(){d.dialog("close")}}]})}function F(a){function b(a){return prspdata.e.vf.findIndex(function(b){return a==b.l})}a=M(a);if(null==a)return!1;PState.set(PSTATE_PROCESS);jQuery("#filter-frame").hide();k.forEach(function(a){a.f.teardown()});k=[];jQuery("#filter-instances").empty();
a.s.f.forEach(function(a){H(a.id,a.a,null).setState(a.s)});jQuery("#filter-instances").hide();jQuery("#btn-toggle-filters").button(0==a.s.f.length?"disable":"enable");for(var d=0;2>d;d++)if(null!=a.s["h"+d]){var c=a.s["h"+d];v[d]=c.id;var h=H(c.id,null,d);w[d]=h;h.setState(c.s)}else w[d]=null,v[d]=null;var c=f[0],h=f[1],g=!1;PState.set(PSTATE_BUILD);d=b(a.s.v0.l);c?(c.setViz(d,!1),c.selBtns(!1)):(f[0]=PViewFrame(0),c=f[0],c.initDOM(d));null!==a.s.v1?(d=b(a.s.v1.l),h?(h.selBtns(!1),h.setViz(d,!1),
h.setState(a.s.v1.s)):(c.flushLgnd(),f[1]=PViewFrame(1),h=f[1],h.initDOM(d),h.setState(a.s.v1.s),g=!0)):h&&(f[1]=null,jQuery("#view-frame-1").remove(),g=!0);g&&c.resize();c.setState(a.s.v0.s);q(a.n);if(PData.ready()&&x)for(u(),d=0;2>d;d++)null!==v[d]&&L(d);return!0}function G(a,b){var d=prspdata.bClrs[a];d&&0<d.length&&jQuery(b).css("background-color",d)}var f=[null,null],I,k=[],w=[null,null],v=[null,null],x,b,g=null,c=[],l;jQuery("body").addClass("waiting");G("cb","#command-bar");G("fs","#filter-frame");
PState.init();"undefined"!==typeof PMapHub&&PMapHub.init(prspdata.m,prspdata.mg);(function(){function a(a,c){b=document.getElementById(a).innerHTML;dlText[c]=b.trim()}var b;a("dltext-removehideall","rha");a("dltext-showhideall","sha");a("dltext-ok","ok");a("dltext-cancel","cancel");a("dltext-next","next");a("dltext-prev","prev");a("dltext-choose-att","chsatt");a("dltext-seerec","seerec");a("dltext-close","close");a("dltext-add","add");a("dltext-manage","manage");a("dltext-delete","del");a("dltext-edit",
"edit");a("dltext-markers","markers");a("dltext-hint-marker","markersize");a("dltext-hint-text","textsize");a("dltext-xaxis","xaxis");a("dltext-yaxis","yaxis");a("dltext-undefined","undef");a("dltext-orderedby","orderedby");a("dltext-grpblks","grpblks");a("dltext-reset","reset");a("dltext-nofilter","nofilter");a("dltext-dofilters","dofilters");a("dltext-filtered","filtered");b=document.getElementById("dltext-month-names").innerHTML;months=b.trim().split("|");(b=document.getElementById("dltext-d3-local"))&&
(b=b.innerHTML)&&1<b.length&&(localD3=d3.locale(JSON.parse(b)).timeFormat.multi([["%H:%M",function(a){return a.getMinutes()}],["%H:%M",function(a){return a.getHours()}],["%a %d",function(a){return a.getDay()&&1!=a.getDate()}],["%b %d",function(a){return 1!=a.getDate()}],["%B",function(a){return a.getMonth()}],["%Y",function(){return!0}]]))})();xhbtURL=window.location.pathname;xhbtURL=xhbtURL.replace(/\&*prspctv=[\w\-]+/,"");xhbtURL=xhbtURL.replace(/\/$/,"");xhbtURL=xhbtURL.replace(/^\//,"");xhbtURL=
"http://"+window.location.host+"/"+xhbtURL;(function(){var a=_.template(document.getElementById("dltext-filter-template").innerHTML),b=[];prspdata.t.forEach(function(c,e){b.push(a({ti:e,tl:c.def.l}))});I=b.join("&nbsp;")})();(function(){var a;prspdata.t.forEach(function(b,c){var e="";b.def.a.forEach(function(b){a=PData.aByID(b);switch(a.def.t){case "T":case "V":case "N":case "D":e+='<option value="'+b+'">'+a.def.l+"</option>"}});jQuery("#dialog-sortby").append("<b>"+b.def.l+"</b>: <select data-ti="+
c+">"+e+"</select><br/>")})})();"/"!=prspdata.site_url.charAt(prspdata.site_url.length-1)&&(prspdata.site_url+="/");""!=prspdata.e.g.l&&jQuery("#title").text(prspdata.e.g.l);try{var m=window.localStorage;m.setItem("__storage_test__","__storage_test__");m.removeItem("__storage_test__");var T=m.getItem(prspdata.e.id),g=m;0<T.length&&(c=JSON.parse(T))}catch(a){}jQuery("#btn-about").button({icons:{primary:"ui-icon-power"},text:!1}).click(function(a){var b;jQuery("#dialog-about img").removeClass("zoomin");
