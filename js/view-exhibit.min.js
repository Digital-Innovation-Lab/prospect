...
var g=PData.getNumETmplts(),l=0,n,m=0,r,h,q,t,w,p,u,k,y,A,z,D,v,x,B,C,E;x=this.settings.min;"string"==typeof x&&(x=parseInt(x));m=this.settings.max;"string"==typeof m&&(m=parseInt(m));B=m-x;for(l=0;l<g;l++)this.tLCnt[l]=0;for(var G,l=0;l<g;l++)if("disable"!=this.settings.pAtts[l]){G=[];break}l=0;m=-1;a:for(;l<a.l;){if(null==p){do{if(++m==g)break a;r=a.t[m]}while(0==r.n||r.i+r.n==l);p=this.vFrame.getSelLocAtts(m);if(0==p.length){p=null;continue}u=e.vFrame.getSelFeatAtts(m);if(0==u.length){p=null;continue}e.tLCnt[m]=
p.length;t=e.vFrame.getSelLegend(m);w=PData.getAttID(t);k=e.settings.pAtts[m];h=e.settings.lClrs[m];if(D=e.settings.sAtts[m])v=PData.getAttID(D),"number"==typeof v.r.min&&"number"==typeof v.r.max?(C=v.r.min,E=v.r.max-C):D=null}n=a.s[l];q=PData.getRecByIndex(n);var F;G&&(F={id:q.id,c:[],p:q.a[k],l:h});p.forEach(function(a){if(y=q.a[a])if(A=q.a[t],"undefined"!=typeof A&&(A=PData.getAttLgndVal(A,w,u))){if("number"==typeof y[0])D?(v=q.a[D],v="undefined"!=typeof v?Math.floor((v-C)*B/E)+x:x):v=x,z=L.circleMarker(y,
{_aid:n,weight:1,radius:v,fillColor:A,color:"#000",opacity:1,fillOpacity:1}),F&&F.c.push(y);else if(z=2==y.length?L.polyline(y,{_aid:n,weight:1,fillColor:A,color:"#000",opacity:1,fillOpacity:1}):L.polygon(y,{_aid:n,weight:1,fillColor:A,color:"#000",opacity:1,fillOpacity:1}),F){var h=[0,0];y.forEach(function(a){h[0]+=a[0];h[1]+=a[1]});h[0]/=y.length;h[1]/=y.length;F.c.push(h)}z.on("click",c);d.addLayer(z)}});F&&0<F.c.length&&G.push(F);++l==r.i+r.n&&(p=null)}if(G){G.sort(function(a,h){return PData.strcmp(h.id,
a.id)});var I=[];G.forEach(function(a){a.p&&a.p.forEach(function(h){l=_.sortedIndex(G,{id:h},"id");if(l<G.length){var c=G[l];c.id==h&&a.c.forEach(function(h){c.c.forEach(function(c){h[0]==c[0]&&h[1]==c[1]||I.push({p:[h,c],c:a.l})})})}})});I.forEach(function(a){f.addLayer(L.polyline(a.p,{color:a.c,weight:2}))})}};VizMap.prototype.teardown=function(){var a=this.vFrame.getIndex();jQuery("#view-frame-"+a+" div.view-control-bar div.iconbar").remove()};VizMap.prototype.resize=function(){this.lMap.invalidateSize(!1)};
...
c+'"]').val(this.sAtts[c])};
VizCards.prototype.render=function(a){var c=this;this.stream=a;var e=PData.getNumETmplts(),d,f,g,l,n,m,r,h,q,t,w,p,u,k,y=jQuery(this.frameID);y.empty();0<this.recSel.length&&(this.recSel=[]);var A,z="w"+this.settings.w+" h"+this.settings.h;f=a.t[0];for(d=0;d<e;){for(;0==f.n;){if(++d==e)return;f=a.t[d]}f=PData.getETmpltIndex(d);f=PData.getTmpltID(f);m=c.vFrame.getSelFeatAtts(d);0!=m.length&&(y.append('<div class="template-label">'+f.l+'</div><div class="cards" data-ti="'+d+'"></div>'),A=jQuery('div.cards[data-ti="'+
d+'"]'),g=c.vFrame.getSelLegend(d),l=PData.getAttID(g),n=c.settings.iAtts[d],t=c.settings.cnt[d],f=c.sAtts[d],f=PData.getAttID(f),f=PData.orderTBy(f,a,d),f.forEach(function(a){r=PData.getRecByIndex(a.i);w=r.a[g];"undefined"!=typeof w&&(h=PData.getAttLgndRecs(w,l,m,!1))&&(u=c.settings.lOn?'<div class="card-title">'+r.l+"</div>":"",q=!1,p="",t&&0<t.length&&(k=h.b?' style="color:black"':"",t.forEach(function(a){if(w=r.a[a])if(w=PData.procAttTxt(a,w))q=!0,p+=w+"<br/>"})),p=n&&(w=r.a[n])?q?'<div class="card-body"><img src="'+
w+'"/><div class="card-cnt"'+k+">"+p+"</div></div>":'<div class="card-body"><img class="full" src="'+w+'"/></div>':'<div class="card-body"><div class="card-cnt"'+k+">"+p+"</div>",A.append('<div class="card '+z+'" style="background-color:'+h.v+'" data-ai="'+a.i+'">'+u+p+"</div>"))}));f=a.t[++d]}};VizCards.prototype.teardown=function(){jQuery(this.frameID).off("click.vf")};
VizCards.prototype.rerender=function(a){var c=this,e=jQuery(this.frameID+' div.cards[data-ti="'+a+'"]');e.empty();var d,f,g,l,n,m,r,h,q,t,w,p,u,k="w"+this.settings.w+" h"+this.settings.h;d=c.vFrame.getSelFeatAtts(a);0!=d.length&&(f=c.vFrame.getSelLegend(a),g=PData.getAttID(f),l=c.settings.iAtts[a],n=c.settings.cnt[a],m=c.sAtts[a],m=PData.getAttID(m),a=PData.orderTBy(m,c.stream,a),a.forEach(function(a){r=PData.getRecByIndex(a.i);h=r.a[f];"undefined"!=typeof h&&(q=PData.getAttLgndRecs(h,g,d,!1))&&(p=
c.settings.lOn?'<div class="card-title">'+r.l+"</div>":"",t=!1,w="",n&&0<n.length&&(u=q.b?' style="color:black"':"",n.forEach(function(a){if(h=r.a[a])if(h=PData.procAttTxt(a,h))t=!0,w+=h+"<br/>"})),w=l&&(h=r.a[l])?t?'<div class="card-body"><img src="'+h+'"/><div class="card-cnt"'+u+">"+w+"</div></div>":'<div class="card-body"><img class="full" src="'+h+'"/></div>':'<div class="card-body"><div class="card-cnt"'+u+">"+w+"</div>",e.append('<div class="card '+k+(c.isSel(a.i)?" obj-sel":"")+'" style="background-color:'+
q.v+'" data-ai="'+a.i+'">'+p+w+"</div>"))}))};VizCards.prototype.doOptions=function(){var a=this,c=jQuery("#dialog-sortby").dialog({height:220,width:400,modal:!0,buttons:[{text:dlText.ok,click:function(){c.dialog("close");PState.set(PSTATE_BUILD);for(var e=0;e<PData.getNumETmplts();e++){var d=jQuery('#dialog-sortby select[data-ti="'+e+'"]').val();d!=a.sAtts[e]&&(a.sAtts[e]=d,a.rerender(e))}PState.set(PSTATE_READY)}},{text:dlText.cancel,click:function(){c.dialog("close")}}]})};
...
"y axis").call(this.yAxis);this.chart.append("image").attr("id","base-"+e).attr("xlink:href",a.img).attr("x",0).attr("y",0).attr("height",a.dh).attr("width",a.dw);this.settings.lyrs.forEach(function(a,d){c.chart.append("svg").attr("id","ol-"+e+"-"+d).attr("opacity",a.o)});this.settings.lyrs.forEach(function(a,c){d3.xml(a.url,function(a,d){if(!a){var r=d.getElementsByTagName("svg")[0];d3.select("#ol-"+e+"-"+c).node().appendChild(r)}})});this.gRecs=this.chart.append("g").attr("id","recs")};
VizPinboard.prototype.render=function(a){var c=this;this.gRecs.selectAll(".recobj").remove();this.gRecs.selectAll(".recline").remove();0<this.recSel.length&&(this.recSel=[]);var e=PData.getNumETmplts(),d,f,g=0,l,n,m,r,h,q,t,w,p,u,k,y,A,z,D,v;for(d=0;d<e;d++)if("disable"!=this.settings.pAtts[d]){v=[];break}y=this.settings.min;"string"==typeof y&&(y=parseInt(y));f=this.settings.max;"string"==typeof f&&(f=parseInt(f));A=f-y;var x=[];d=0;g=-1;a:for(;d<a.l;){if(null==q){do{if(++g==e)break a;l=a.t[g]}while(0==
l.n||l.i+l.n==d);q=this.vFrame.getSelLocAtts(g);if(0==q.length){q=null;continue}q=q[0];t=c.vFrame.getSelFeatAtts(g);if(0==t.length){q=null;continue}r=c.vFrame.getSelLegend(g);h=PData.getAttID(r);w=c.settings.pAtts[g];n=c.settings.lClrs[g];if(k=c.settings.sAtts[g])m=PData.getAttID(k),"number"==typeof m.r.min&&"number"==typeof m.r.max?(z=m.r.min,D=m.r.max-z):k=null}f=a.s[d];m=PData.getRecByIndex(f);var B;v&&(B={id:m.id,c:null,p:m.a[w],l:n});p=m.a[q];"undefined"!=typeof p&&(u=m.a[r],"undefined"!=typeof u&&
(u=PData.getAttLgndVal(u,h,t)))&&(k?(m=m.a[k],m="undefined"!=typeof m?Math.floor((m-z)*A/D)+y:y):m=y,x.push({ai:f,v:u,x:p[0],y:p[1],r:m}),B&&(B.c=p));B&&B.c&&v.push(B);++d==l.i+l.n&&(q=null)}this.gRecs.selectAll(".recobj").data(x).enter().append("circle").attr("class","recobj").attr("cx",function(a){return c.xScale(a.x)}).attr("cy",function(a){return c.yScale(a.y)}).attr("r",function(a){return c.yScale(a.r)}).style("fill",function(a){return a.v}).on("click",function(a,h){var q=c.toggleSel(a.ai);d3.select(this).classed("obj-sel",
q)});if(v){v.sort(function(a,h){return PData.strcmp(h.id,a.id)});var C=[];v.forEach(function(a){a.p&&a.p.forEach(function(h){d=_.sortedIndex(v,{id:h},"id");if(d<v.length){var c=v[d];c.id==h&&C.push({f:a.c,t:c.c,c:a.l})}})});this.gRecs.selectAll(".recline").data(C).enter().append("line").attr("class","recline").attr("x1",function(a){return c.xScale(a.f[0])}).attr("y1",function(a){return c.yScale(a.f[1])}).attr("x2",function(a){return c.xScale(a.t[0])}).attr("y2",function(a){return c.yScale(a.t[1])}).attr("stroke",
...
q}else c.xScale.domain([d.minDate,d.maxDate]);c.xScale.range([0,c.w]);c.g=d.chart.append("g").attr("id",c.svgID.substring(1)).attr("class","tl-band").attr("width",c.w);d.bands[a]=c;d.cmpnts.push(c)}function c(a){var c=d.bands[a],h=d3.svg.axis().scale(c.xScale).orient("bottom").tickSize(6,0);localD3&&h.tickFormat(localD3);var q=c.g.append("g").attr("id","axis-"+n+"-"+a).attr("class","axis").style("font-size","10px");a={redraw:function(){q.call(h)}};c.parts.push(a);d.cmpnts.push(a)}function e(a){var c=
d.bands[a],h;h=1==a?32:16;c.labels=[{name:"s-"+n+"-"+a,x:function(){return 0},left:function(){return 0},anchor:"start",tDelta:2,whichDate:function(a,c){return a}},{name:"e-"+n+"-"+a,x:function(){return c.l+c.w},left:function(){return c.l+c.w-d.settings.xLbl},anchor:"end",tDelta:-3,whichDate:function(a,c){return c}}];var q=d3.select(c.svgID).selectAll(".bLblCntr").data(c.labels).enter().append("g").attr("class","bLblCntr");q.append("rect").attr("class","bLbl").attr("id",function(a){return"rect-"+a.name}).attr("x",
function(a){return a.left()}).attr("width",d.settings.xLbl).attr("height",h);var t=q.append("text").attr("class","bMinMaxLbl").attr("id",function(a){return"txt-"+a.name}).attr("x",function(a){return a.x()+a.tDelta}).attr("y",12).attr("text-anchor",function(a){return a.anchor}),e={redraw:function(){var a=c.xScale.domain(),h=a[0],q=a[1];t.text(function(a){return a.whichDate(h,q).getUTCFullYear()})}};c.parts.push(e);d.cmpnts.push(e);if(1==a){var p=q.append("text").attr("class","bMinMaxLbl").attr("id",
function(a){return"m-txt-"+a.name}).attr("x",function(a){return a.x()+a.tDelta}).attr("y",h-4).attr("text-anchor",function(a){return a.anchor});a={redraw:function(){var a=c.xScale.domain(),h=a[0],q=a[1];p.text(function(a){return q.getUTCFullYear()-h.getUTCFullYear()>d.threshold?"":months[a.whichDate(h,q).getMonth()]})}};c.parts.push(a);d.cmpnts.push(a)}c.labelSVGs=q}var d=this;"number"!=typeof this.settings.xLbl&&(this.settings.xLbl=parseInt(this.settings.xLbl));this.brushSVG=this.brush=null;var f=
this.settings;"string"===typeof f.bHt&&(f.bHt=parseInt(f.bHt,10));(function(){var a,c,h,q,t,e;f.dAtts.forEach(function(d){null!=d&&"disable"!=d&&(d=PData.getAttID(d))&&(null==a||d.r.min.y<a?(a=d.r.min.y,"undefined"!=typeof d.r.min.m?(c=d.r.min.m,h="undefined"!=typeof d.r.min.d?d.r.min.d:1):h=c=1):d.r.min.y==a&&"undefined"!=typeof d.r.min.m&&(d.r.min.m<c?(c=d.r.min.m,h="undefined"!=typeof d.r.min.d?d.r.min.d:1):d.r.min.m==c&&"undefined"!=typeof d.r.min.d&&d.r.min.d<h&&(h=d.r.min.d)),null==q?"undefined"==
typeof d.r.max.y?(q=TODAY.getUTCFullYear(),t=TODAY.getMonth()+1,e=TODAY.getDate()):(q=d.r.max.y,"undefined"!=typeof d.r.max.m?(t=d.r.max.m,e="undefined"!=typeof d.r.max.d?d.r.max.d:mnthDays[t-1]):(t=12,e=31)):d.r.max.y>q?(q=d.r.max.y,"undefined"!=typeof d.r.max.m?(t=d.r.max.m,e="undefined"!=typeof d.r.max.d?d.r.max.d:mnthDays[t-1]):(t=12,e=31)):d.r.max.y==q&&"undefined"!=typeof d.r.max.m&&(d.r.max.m>t?(t=d.r.max.m,e="undefined"!=typeof d.r.max.d?d.r.max.d:mnthDays[t-1]):d.r.max.m==t&&"undefined"!=
typeof d.r.max.d&&d.r.max.d>e&&(e=d.r.max.d)))});d.minDate=0<f.from.length?PData.parseDate(f.from,!1):PData.date3Nums(a,c,h,!1);d.maxDate=0<f.to.length?PData.parseDate(f.to,!0):PData.date3Nums(q,t,e,!0);d.instGap=.015*(d.maxDate-d.minDate)})();var g=d.getWidths(),l=d3.select(this.frameID).append("svg").attr("class","tl-vf").attr("width",g[1]);(function(){var a=[];f.lgnds.forEach(function(c){c.forEach(function(c){a.push(c)})});var a=_.uniq(a),c=[];a.forEach(function(a){(a=PData.getAttID(a))&&a.l.forEach(function(a){c.push(a.v)})});
...
"100%").attr("stop-color","#C0C0C0");q=h.append("linearGradient").attr("id",d+"-fb");q.append("stop").attr("offset","0%").attr("stop-color","#C0C0C0");q.append("stop").attr("offset","5%").attr("stop-color",a);q.append("stop").attr("offset","95%").attr("stop-color",a);q.append("stop").attr("offset","100%").attr("stop-color","#C0C0C0")});h.append("filter").attr("id","xortext").append("feComposite").attr("operator","xor")})();var n=this.vFrame.getIndex();l.append("clipPath").attr("id","tl-clip-"+n).append("rect").attr("width",
g[1]);d.chart=l.append("g").attr("class","chart").attr("clip-path","url(#tl-clip-"+n+")");d.instRad=f.bHt/2-1;d.bands=Array(2);d.cmpnts=[];a(0);a(1);c(0);c(1);e(0);e(1)};
VizTime.prototype.render=function(a){function c(a,c){var h=a.s-c.s;if(0>h)return 1;if(0<h)return-1;h=c.e-a.e;return 0>h?1:0<h?-1:0}function e(a){function c(a){a=g.toggleSel(a.ai);d3.select(this).classed("obj-sel",a)}var h,q,d,e,p=g.bands[a];a?(h=g.bands[0],p.t=h.t+h.h+37,h=q=d=g.instRad,e=2*g.instRad+3):(p.t=0,h=q=d=1);p.h=n*p.tHt+2;p.g.attr("transform","translate(0,"+p.t+")").attr("height",p.h);var u,k;a&&(u=.75*p.iHt+"px",k=.8*p.iHt);var f;f=d3.select(p.svgID).selectAll(".lgBd").remove();f=d3.select(p.svgID).selectAll(".lgBd").data(g.lgBds).enter().append("svg").attr("class",
"lgBd").attr("y",function(a){return p.yScale(a.t)}).attr("height",function(a){return p.yScale(a.h)});f.append("rect").attr("width","100%").attr("height","100%").attr("fill",function(a){return a.d.v});1==a&&f.append("text").attr("class","lgBdLbl").attr("x",2).attr("y",k).attr("fill",function(a){return a.d.b?"#000000":"#FFFFFF"}).style("font-size",u).text(function(a){return a.d.l});var l;d3.select(p.svgID).selectAll(".event").remove();l=d3.select(p.svgID).selectAll(".event").data(g.events).enter().append("svg").attr("class",
function(a){return a.f&EVENT_INSTANT?"event instant":"event range"}).attr("y",function(a){return p.yScale(a.t)}).attr("height",p.iHt);if(1==a)l.on("click",c);var z=d3.select(p.svgID).selectAll(".range");z.append("rect").attr("width","100%").attr("height","100%").attr("fill",function(c){return 1==a&&c.f&(EVENT_F_START|EVENT_F_END)?(c.f&(EVENT_F_START|EVENT_F_END))==(EVENT_F_START|EVENT_F_END)?"url("+c.c.v+"-fb)":(c.f&EVENT_F_START)===EVENT_F_START?"url("+c.c.v+"-fs)":"url("+c.c.v+"-fe)":c.c.v});1==
a&&z.append("text").attr("class","rangeLbl").attr("x",4).attr("y",k).attr("fill",function(a){return a.c.b?"#000000":"#FFFFFF"}).style("font-size",u).text(function(a){return a.l});z=d3.select(p.svgID).selectAll(".instant");z.append("circle").attr("cx",h).attr("cy",q).attr("r",d).attr("fill",function(a){return a.c.v});1==a&&z.append("text").attr("class","instantLbl").attr("x",e).attr("y",k).style("font-size",u).text(function(a){return a.l});p.redraw=function(){f.attr("x",function(a){return p.xScale(a.s)}).attr("width",
function(a){return p.xScale(a.e)-p.xScale(a.s)});l.attr("x",function(a){return p.xScale(a.s)}).attr("width",function(a){return p.xScale(a.e)-p.xScale(a.s)});p.parts.forEach(function(a){a.redraw()})}}function d(a){a=g.bands[a];d3.select(a.svgID).selectAll(".axis").attr("transform","translate(0,"+a.h+")")}function f(a){a=g.bands[a];d3.select(a.svgID).selectAll(".bLblCntr").attr("transform","translate(0,"+(a.h+1).toString()+")")}var g=this,l=g.vFrame.getIndex();0<this.recSel.length&&(this.recSel=[]);
g.events=[];g.lgBds=[];var n=0;(function(){for(var d=PData.getNumETmplts(),e=0,h,q,t,f,p,u,k,l;e<d;){for(h=a.t[e];0==h.n;){if(++e==d)return;h=a.t[e]}t=g.vFrame.getSelLocAtts(e);if(0!=t.length&&(t=g.vFrame.getSelFeatAtts(e),0!=t.length)){l=g.vFrame.getSelLegend(e);k=PData.getAttID(l);f=g.settings.dAtts[e];for(var A,z,D,v,x,B,C,E,G=[],F=h.i;F<h.i+h.n;F++)q=a.s[F],E=PData.getRecByIndex(q),p=E.a[l],"undefined"!=typeof p&&(p=PData.getAttLgndRecs(p,k,t,!1))&&(u=E.a[f])&&(B=u.min.f?EVENT_F_START:0,A=u.min.y,
"undefined"==typeof u.min.m?D=z=1:(z=u.min.m,D="undefined"==typeof u.min.d?1:u.min.d),v=PData.date3Nums(A,z,D,!1),"undefined"==typeof u.max?(B|=EVENT_INSTANT,x=v.getTime()+g.instGap):"open"==u.max?x=TODAY:(u.max.f&&(B|=EVENT_F_END),A=u.max.y,"undefined"==typeof u.max.m?(z=12,D=31):(z=u.max.m,D="undefined"==typeof u.max.d?mnthDays[z-1]:u.max.d),x=PData.date3Nums(A,z,D,!0)),G.push({s:v,e:x,ai:q,f:B,c:p,l:E.l,t:0}));G.sort(c);var I=[],J;G.forEach(function(a){for(J=0;J<I.length&&!(a.e<I[J]);J++);a.t=
J+n+1;I[J]=a.s});h=PData.getAttID(f);h.l.forEach(function(a){C=a.d;A=C.min.y;"undefined"==typeof C.min.m?D=z=1:(z=C.min.m,D="undefined"==typeof C.min.d?1:C.min.d);v=PData.date3Nums(A,z,D,!1);"undefined"==typeof C.max.y?x=TODAY:(A=C.max.y,"undefined"==typeof C.max.m?(z=12,D=31):(z=C.max.m,D="undefined"==typeof C.max.d?mnthDays[z-1]:C.max.d),x=PData.date3Nums(A,z,D,!0));g.lgBds.push({s:v,e:x,t:n,h:I.length+1,d:a})});n+=I.length+1;g.events=g.events.concat(G)}e++}})();g.getWidths();e(0);e(1);(function(){var a=
...
VizDirectory.prototype.render=function(a){var c=PData.getNumETmplts(),e=0,d,f,g,l,n,m,r=jQuery(this.frameID);r.empty();0<this.recSel.length&&(this.recSel=[]);this.stream=a;for(d=a.t[0];e<c;){for(;0==d.n;){if(++e==c)return;d=a.t[e]}d=PData.getETmpltIndex(e);d=PData.getTmpltID(d);r.append('<div class="template-label">'+d.l+'</div><table cellspacing="0" class="viz-directory" data-ti='+e+"></table>");f=r.find('table[data-ti="'+e+'"]');g=this.settings.cnt[e];m="<thead><tr>";g.forEach(function(a){a=PData.getAttID(a);
m+="<th>"+a.def.l+"</th>"});f.append(m+"<tr></thead><tbody></tbody>");f=f.find("tbody");d=this.sAtts[e];d=PData.getAttID(d);d=PData.orderTBy(d,this.stream,e);d.forEach(function(a){n=PData.getRecByIndex(a.i);m='<tr data-id="'+n.id+'" data-ai='+a.i+">";g.forEach(function(a){l=n.a[a];"undefined"!=typeof l?m=(l=PData.procAttTxt(a,l))?m+("<td>"+l+"</td>"):m+"<td></td>":m+="<td></td>"});f.append(m+"</tr>")});d=a.t[++e]}};
VizDirectory.prototype.rerender=function(a){var c=this,e,d,f,g,l,n=jQuery(this.frameID+' table.viz-directory[data-ti="'+a+'"] tbody');n.empty();tRec=this.stream.t[a];e=c.settings.cnt[a];d=PData.getAttID(c.sAtts[a]);PData.orderTBy(d,c.stream,a).forEach(function(a){g=PData.getRecByIndex(a.i);l="<tr "+(c.isSel(a.i)?'class="obj-sel" ':"")+'data-id="'+g.id+'" data-ai='+a.i+">";e.forEach(function(a){f=g.a[a];"undefined"!=typeof f?l=(f=PData.procAttTxt(a,f))?l+("<td>"+f+"</td>"):l+"<td></td>":l+="<td></td>"});
...
VizTextStream.prototype.render=function(a){var c=this,e=PData.getNumETmplts(),d=0,f,g,l,n,m,r,h,q,t,w,p,u,k,y,A,z,D,v=jQuery(this.frameID);v.empty();0<this.recSel.length&&(this.recSel=[]);z=this.settings.max-this.settings.min;for(g=a.t[0];d<e;){for(;0==g.n;){if(++d==e)return;g=a.t[d]}t=c.vFrame.getSelLocAtts(d);if(0!=t.length&&(t=c.vFrame.getSelFeatAtts(d),0!=t.length)){w=c.vFrame.getSelLegend(d);p=PData.getAttID(w);v.append('<div class="viz-textstream" data-ti='+d+"></div>");l=v.find("div.viz-textstream[data-ti="+
d+"]");f=PData.getETmpltIndex(d);f=PData.getTmpltID(f);l.append('<div class="template-label">'+f.l+"</div>");q=c.settings.cnt[d];if(y=c.settings.sAtts[d])k=PData.getAttID(y),"number"==typeof k.r.min&&"number"==typeof k.r.max?A=k.r.max-k.r.min:y=null;q&&(f=PData.getAttID(c.settings.order[d]),f=PData.orderTBy(f,a,d),f.forEach(function(a){n=PData.getRecByIndex(a.i);m=n.a[w];"undefined"!=typeof m&&(u=PData.getAttLgndRecs(m,p,t,!1))&&((r=n.a[q])&&(r=PData.procAttTxt(q,r)),r&&(y?(h=n.a[y],h="undefined"!=
typeof h?Math.floor((h-k.r.min)*z/A)+c.settings.min:c.settings.min):h=c.settings.min,D=u.b?";background-color:black":"",l.append('<div class="recitem" data-ai='+a.i+' style="color:'+u.v+D+";font-size:"+h+'px">'+r+"</div>")))}))}d++}};VizTextStream.prototype.teardown=function(){jQuery(this.frameID).off("click.vf")};
...
VizNetWheel.prototype.render=function(a){var c=this,e=[],d,f,g;this.svg.selectAll(".node").remove();this.svg.selectAll(".link").remove();this.inc=360/(a.l+a.t.length);f=Math.max((14*a.l+20)/(2*Math.PI),30);var l=this.settings.lw;"string"==typeof l&&(l=parseInt(l));this.cr=l+12+f;l=2*this.cr;this.svg.attr("width",l).attr("height",l);this.center.attr("transform","translate("+this.cr+","+this.cr+")");0<this.recSel.length&&(this.recSel=[]);var n={children:[]};(function(){var d,e=0,h=0,q,t,f,p=[],u=null,
k,g;d=a.t[0];g=c.vFrame.getSelLegend(0);k=PData.getAttID(g);u=c.vFrame.getSelFeatAtts(0);a:for(;h<a.l;){for(;0==d.n||d.i+d.n==h||0==u.length;){0<p.length&&(n.children.push({ti:e,children:p}),p=[]);if(++e==PData.getNumETmplts())break a;p=[];d=a.t[e];g=c.vFrame.getSelLegend(e);k=PData.getAttID(g);u=c.vFrame.getSelFeatAtts(e)}f=a.s[h];q=PData.getRecByIndex(f);t=q.a[g];"undefined"!=typeof t&&(fData=PData.getAttLgndRecs(t,k,u,!1))&&p.push({r:q,ai:f,c:fData.v,children:[]});h++}0<p.length&&n.children.push({ti:e,
...
180*Math.PI});n.children.forEach(function(a){var d=c.settings.pAtts[a.ti],h;a.children.forEach(function(a){d.forEach(function(c){h=a.r.a[c.pid];"undefined"!=typeof h&&h.forEach(function(h){var d=!1,f,k=0;a:for(;k<n.children.length;k++)for(var g=n.children[k],l=0;l<g.children.length;l++)if(f=g.children[l],f.r.id==h){d=!0;break a}d&&e.push({source:a,target:f,c:c.clr})})})})});d=this.center.append("g").selectAll(".link").data(f(e)).enter().append("path").each(function(a){a.s=a[0];a.t=a[a.length-1]}).attr("class",
...
d){l[c][d]=PData.intersect(a.i,h.i)})});for(var m=Array(g.c.length),r=0;r<g.c.length;r++){for(var h=0,n=0;n<a.c.length;n++)0<l[n][r].length&&h++;m[r]=h-1}for(var q=Array(g.c.length),r=0;r<g.c.length;r++)q[r]=0;var t=0;a.c.forEach(function(h,p){for(var f=0,k=0;k<g.c.length;k++)0<l[p][k].length&&f++;--f;var n=h.i.length*e/a.t,r=0,z=0;g.c.forEach(function(k,v){var x=l[p][v];if(0<x.length){var B=x.length*e,C=B/a.t,B=B/g.t,E=k.i.length*e/g.t;c.ints.push({i:x,c:h.c,l:h.l+" > "+k.l,x1:5+t*e/a.t+(f?(n-C)*
r/f:0),x2:5+z*e/g.t+(m[v]?(E-B)*q[v]/m[v]:0),w1:C,w2:B,y1:a.y+10,y2:g.y-1});r++;q[v]++}z+=k.i.length});t+=h.i.length})}});this.flowG.selectAll(".flow").data(c.ints).enter().append("path").attr("class","flow").attr("d",function(a){return"M "+a.x1+" "+a.y1+"  L "+(a.x1+a.w1)+" "+a.y1+" L "+(a.x2+a.w2)+" "+a.y2+" L "+a.x2+" "+a.y2+" L "+a.x1+" "+a.y1}).attr("fill",function(a){return a.c}).on("click",function(a,e){var g=c.fSel.length,l=_.sortedIndex(c.fSel,e);c.fSel[l]==e?(d3.select(this).classed("obj-sel",
...
this.rCats.length]).rangeRound([0,e]),n=d3.scale.linear().domain([0,100]).range([f,0]),d=d3.scale.ordinal().rangeRoundBands([0,e]);d.domain(this.rCats.map(function(a){return a.l}));var d=d3.svg.axis().scale(d).orient("bottom"),m=d3.svg.axis().scale(n).orient("left").ticks(4),e=d3.select(c.get(0)).append("svg").attr("width",e+D3FG_MARGINS.left+D3FG_MARGINS.right).attr("height",f+D3FG_MARGINS.top+D3FG_MARGINS.bottom).append("g").attr("transform","translate("+D3FG_MARGINS.left+","+D3FG_MARGINS.top+")");
e.append("g").attr("class","x axis").attr("transform","translate(0,"+f+")").call(d);e.append("g").attr("class","y axis").call(m);e.selectAll(".bar").data(this.rCats).enter().append("rect").attr("class","bar").attr("x",function(a,c){return l(c)+2}).attr("y",function(a){return n(100)}).attr("fill",function(a){return a.c}).attr("height",function(a){return f-n(100)}).attr("width",g-4);a.brush=d3.svg.brush().x(l).extent([0,this.rCats.length]).on("brushend",function(){if(d3.event.sourceEvent){var c=a.brush.extent(),
...
var l=d3.svg.axis().scale(l).orient("bottom"),n=d3.svg.axis().scale(g).orient("left").ticks(4),m=this.insertPt(),r=_.template(document.getElementById("dltext-filter-date-ctrl").innerHTML);m.append(r({id:this.id}));m.find("input[name=dctrl-"+a.id+"]").change(function(){a.isDirty(!0)});d=d3.select(m.get(0)).append("svg").attr("width",d+D3FG_MARGINS.left+D3FG_MARGINS.right).attr("height",c+D3FG_MARGINS.top+D3FG_MARGINS.bottom).append("g").attr("transform","translate("+D3FG_MARGINS.left+","+D3FG_MARGINS.top+
")");d.append("g").attr("class","x axis").attr("transform","translate(0,"+c+")").call(l);d.append("g").attr("class","y axis").call(n);d.selectAll(".bar").data(this.rCats).enter().append("rect").attr("class","bar").attr("x",function(a,c){return f(c)+2}).attr("y",function(a){return g(100)}).attr("fill",function(a){return a.c}).attr("height",function(a){return c-g(100)}).attr("width",e-4);a.brush=d3.svg.brush().x(f).extent([0,this.rCats.length]).on("brushend",function(){if(d3.event.sourceEvent){var c=
...
t)throw Error("Transcript excerpt error: Cannot find timestamp "+widgetData.extract[0]);var k=a.indexOf(widgetData.extract[1]);if(-1==k)throw Error("Transcript excerpt error: Cannot find timestamp "+widgetData.extract[1]);a=a.substring(t-1,k-1)}if(a=c(a).split(/\r\n|\r|\n/g)){var p=jQuery("#xscript-tbl"),f=0,g,u=0,z=0,w="";_.each(a,function(a){a=c(a);1<a.length&&("["===a.charAt(0)&&"0"<=a.charAt(1)&&"9">=a.charAt(1)?(g=h(a),0<w.length&&(z&&q.push({s:u,e:g}),p.append('<div class="row"><div class="timecode" data-timecode="'+
u+'" data-tcindex="'+f++ +'">'+z+'</div><div class="xscript">'+w+"</div></div>"),w=""),z=a,u=g):(0<w.length&&(w+="<br/>"),w+=a))});0<w.length&&(q.push({s:u,e:324E5}),p.append('<div class="row"><div class="timecode" data-timecode="'+u+'" data-tcindex="'+f+'">'+z+'</div><div class="xscript">'+w+"</div></div>"));if("undefined"!=typeof y&&null!=y){var F=new XMLHttpRequest;F.onload=function(a){d(F.responseText,p)};F.open("GET",y,!0);F.send()}}}function q(a){var c,h=widgetData.tcIndex;_.find(widgetData.tcArray,
...
endSeconds:widgetData.eTime/1E3})}}})}function p(){function a(){widgetData.sTime=widgetData.eTime=null;var c;(c=prspdata.e.i.t.tcAtts[k])&&(c=r.a[c])&&""!=c&&(c=c.split("-"),widgetData.extract=c,widgetData.sTime=h(c[0]),widgetData.eTime=h(c[1]))}var c=m[K];r=PData.getRecByIndex(c);var d=" "+r.l+" ("+(K+1)+"/"+m.length+") ";jQuery("#inspect-name").text(d);var k=PData.aIndex2Tmplt(c);w.empty();prspdata.e.i.modal.atts[k].forEach(function(a){var h=PData.getRecAtt(c,a,!1);h&&(a=PData.getAttID(a),h="_"==
a.def.l.charAt(0)?"<div>"+h+"</div>":'<div><span class="att-label">'+a.def.l+":</span> "+h+"</div>",w.append(h))});l=null;n=0;widgetData.extract=null;widgetData.xscriptOn=!1;widgetData.playing=!1;if(prspdata.e.i.modal.scOn&&(l=prspdata.e.i.sc.atts[k])&&(d=r.a[l])){var f=!0;a();n=1;w.append('<iframe id="sc-widget" class="player" width="95%" height="150" src="http://w.soundcloud.com/player/?url='+d+'"></iframe></p>');var g=SC.Widget(document.getElementById("sc-widget"));widgetData.widget=g;g.bind(SC.Widget.Events.READY,
function(){g.play();g.bind(SC.Widget.Events.PLAY,function(){widgetData.playing=!0});g.bind(SC.Widget.Events.PAUSE,function(){widgetData.playing=!1});g.bind(SC.Widget.Events.PLAY_PROGRESS,function(a){f&&(g.pause(),f=!1,widgetData.playing=!1);widgetData.extract&&(a.currentPosition<widgetData.sTime?g.seekTo(widgetData.sTime):a.currentPosition>widgetData.eTime&&(g.pause(),widgetData.playing=!1));widgetData.playing&&widgetData.xscriptOn&&q(a.currentPosition)});g.bind(SC.Widget.Events.FINISH,function(){widgetData.playing=
!1})})}if(0==n&&prspdata.e.i.modal.ytOn&&(l=prspdata.e.i.yt.atts[k])&&(d=r.a[l])){a();widgetData.ytCode=d;w.append('<div id="yt-widget"></div>');widgetData.ytCall=t;if(widgetData.ytLoaded)t();else{widgetData.ytLoaded=!0;d=document.createElement("script");d.src="https://www.youtube.com/iframe_api";var u=document.getElementsByTagName("script")[0];u.parentNode.insertBefore(d,u)}n=2}if(prspdata.e.i.modal.tOn&&(d=prspdata.e.i.t.t1Atts[k])&&""!==d&&"disable"!=d&&(d=r.a[d])){0<n&&w.append("<div>"+document.getElementById("dltext-sync-xscript").innerHTML+
"</div>");w.find("#xscript-tbl").remove();w.append('<div id="xscript-tbl"><div>');widgetData.xscriptOn=!0;jQuery("#xscript-tbl").click(function(a){if(n&&jQuery(a.target).hasClass("timecode"))switch(a=jQuery(a.target).data("timecode"),n){case 1:widgetData.playing||(widgetData.playing=!0,widgetData.widget.play());widgetData.widget.seekTo(a);break;case 2:widgetData.playing||(widgetData.playing=!0,widgetData.widget.playVideo()),widgetData.widget.seekTo(a/1E3)}});y=null;(u=prspdata.e.i.t.t2Atts[k])&&""!==
u&&"disable"!=u&&(y=r.a[u]);var z=new XMLHttpRequest;z.onload=function(a){e(z.responseText)};z.open("GET",d,!0);z.send()}}function f(a){a=K+a;-1==a?a=m.length-1:a==m.length&&(a=0);a!=K&&(K=a,p())}function g(a){f(-1)}function u(a){f(1)}var w=jQuery("#inspect-content"),l=null,n=0,y,m=null;k&&(m=k.getSel());if(null!=m&&0!=m.length){var A,r,K=0;p();jQuery("#btn-inspect-left").click(g);jQuery("#btn-inspect-right").click(u);var H=450,M=400;prspdata.e.i.modal.scOn&&(H=550);prspdata.e.i.modal.ytOn&&(H=Math.max(H,
475),M=500);prspdata.e.i.modal.tOn&&(M+=100,prspdata.e.i.modal.t2On?(H=Math.max(750,Math.floor(.8*jQuery(document).width())),H=Math.min(900,H)):H=Math.max(H,550));A=jQuery("#dialog-inspector").dialog({width:H,height:M,modal:!0,buttons:[{text:dlText.seerec,click:function(){window.open(prspdata.site_url+"?p="+r.wp,"_blank")}},{text:dlText.close,click:function(){switch(n){case 1:null!=widgetData.widget&&widgetData.playing&&widgetData.widget.pause();widgetData.playing=!1;widgetData.widget=null;break;
case 2:widgetData.ytCall=null,null!=widgetData.widget&&widgetData.playing&&widgetData.widget.stopVideo(),widgetData.widget=null,widgetData.playing=!1,null!=widgetData.timer&&(window.clearInterval(widgetData.timer),widgetData.timer=null)}jQuery("#btn-inspect-left").off("click");jQuery("#btn-inspect-right").off("click");A.dialog("close")}}]});a.preventDefault()}}function g(a){var h=jQuery(c()+" .view-control-bar");a?(h.find(".osel").button("enable"),h.find(".osel").addClass("pulse"),h.find(".xsel").button("enable")):
(h.find(".osel").button("disable"),h.find(".osel").removeClass("pulse"),h.find(".xsel").button("disable"))}function l(a){PState.set(PSTATE_UPDATE);k&&k.clearSel();g(!1);PState.set(PSTATE_READY);a.preventDefault()}function n(a){k&&k.doOptions();a.preventDefault()}function m(a){var c=jQuery("#dialog-vnotes").dialog({width:300,height:300,modal:!0,buttons:[{text:dlText.ok,click:function(){c.dialog("close")}}]});a.preventDefault()}function r(a,h){jQuery(c()+' div.lgnd-container div.lgnd-scroll div.lgnd-template[data-index="'+
a+'"] div.lgnd-group input.lgnd-entry-check').prop("checked",h)}function h(a){var h=jQuery(a.target).closest("div.lgnd-template").data("index"),d=a.target.className;switch(d){case "lgnd-update":k&&A&&(PState.set(PSTATE_BUILD),g(!1),k.render(A),PState.set(PSTATE_READY));break;case "lgnd-entry-check":d=jQuery(a.target).closest("div.lgnd-entry");a=jQuery(a.target).is(":checked");d.hasClass("lgnd-sh")?r(h,a):d.hasClass("lgnd-locate")?d.data("id"):d.hasClass("lgnd-value")&&d.data("index");break;case "lgnd-viz":case "lgnd-value-title":d=
jQuery(a.target).closest("div.lgnd-entry");d.hasClass("lgnd-locate")?(a=d.data("id"),jQuery(c()+' div.lgnd-container div.lgnd-scroll div.lgnd-template[data-index="'+h+'"] div.lgnd-locate input.lgnd-entry-check').prop("checked",!1),jQuery(c()+' div.lgnd-container div.lgnd-scroll div.lgnd-template[data-index="'+h+'"] div.lgnd-locate[data-id="'+a+'"] input.lgnd-entry-check').prop("checked",!0)):d.hasClass("lgnd-value")&&(a=d.data("index"),jQuery(c()+' div.lgnd-container div.lgnd-scroll div.lgnd-template[data-index="'+
h+'"] div.lgnd-group input.lgnd-entry-check').prop("checked",!1),jQuery(c()+' div.lgnd-container div.lgnd-scroll div.lgnd-template[data-index="'+h+'"] div.lgnd-group div.lgnd-value[data-index="'+a+'"] input.lgnd-entry-check').prop("checked",!0));break;case "lgnd-template":case "lgnd-select":case "":break;default:d.match(/lgnd-sh/i)&&(d=jQuery(a.target).find("input.lgnd-entry-check"),a=!d.is(":checked"),d.prop("checked",a),r(h,a))}}function q(a){var c=jQuery(a.target).closest("div.lgnd-template").data("index");
a=jQuery(a.target).val();t(c,a)}function t(a,h){var d,e=jQuery(c()+' div.lgnd-container div.lgnd-scroll div.lgnd-template[data-index="'+a+'"] div.lgnd-group');e.empty();y[a]=h;PData.getAttID(h).l.forEach(function(a,c){d='<div class="lgnd-value lgnd-entry" data-index="'+c+'"><input type="checkbox" checked="checked" class="lgnd-entry-check"/><div class="lgnd-viz" style="background-color: '+a.v+'"> </div> <span class="lgnd-value-title">'+a.l+"</span></div>";e.append(d);a.z&&0<a.z.length&&a.z.forEach(function(a,
h){d='<div class="lgnd-value lgnd-entry" data-index="'+c+","+h+'"><input type="checkbox" checked="checked" class="lgnd-entry-check"/>';d=a.v&&""!=a.v?d+('<div class="lgnd-viz" style="background-color: '+a.v+'"></div>'):d+'<div class="lgnd-viz lgnd-viz-empty"></div>';d+=' <span class="lgnd-value-title">&raquo; '+a.l+"</span></div>";e.append(d)})});jQuery(c()+' div.lgnd-container div.lgnd-scroll div.lgnd-template[data-index="'+a+'"] div.lgnd-sh input').prop("checked",!0)}function w(h,d){var e=PData.getVizIndex(h);
k&&(k.teardown(),k=null);var f=jQuery(c());f.find("div.viz-content div.viz-result").empty();var w;switch(e.vf){case "M":w=new VizMap(p,e.c);break;case "C":w=new VizCards(p,e.c);break;case "P":w=new VizPinboard(p,e.c);break;case "T":w=new VizTime(p,e.c);break;case "D":w=new VizDirectory(p,e.c);break;case "t":w=new VizTextStream(p,e.c);break;case "S":w=new VizStackChart(p,e.c);break;case "N":w=new VizNetWheel(p,e.c);break;case "F":w=new VizFlow(p,e.c)}u=h;var l=w.flags();l&V_FLAG_HSCRL?(f.find("div.viz-content").addClass("h-scroll"),
...
f.find("div.viz-result").addClass("viz-max-h"));y=[];if(l&V_FLAG_LGND){f.find(".hslgnd").button("enable");var n=f.find("div.lgnd-container div.lgnd-scroll");n.empty();if(l&V_FLAG_SLGND){var m=w.getFeatureAtts(),F=PData.getAttID(m);n.append('<div class="lgnd-template" data-index="0"><div class="lgnd-title">'+F.def.l+'</div><div class="lgnd-entry lgnd-sh"><input type="checkbox" checked="checked" class="lgnd-entry-check"/>'+dlText.sha+'</div><div class="lgnd-group"></div></div>');y.push(m);t(0,m)}else prspdata.e.g.ts.forEach(function(a,
c){var h=PData.getTmpltID(a),d=w.getLocAtts(c);if(d&&0<d.length||!(l&V_FLAG_LOC)){var e=jQuery('<div class="lgnd-template" data-index="'+c+'"><div class="lgnd-title">'+h.l+"</div></div>");d&&d.forEach(function(a,c){var h=PData.getAttID(a);e.append('<div class="lgnd-entry lgnd-locate" data-id="'+a+'"><input type="checkbox" checked="checked" class="lgnd-entry-check"/><span class="lgnd-value-title">'+h.def.l+"</span></div>")});var h=w.getFeatureAtts(c),k='<select class="lgnd-select">';h.forEach(function(a,
c){var h=PData.getAttID(a);k+='<option value="'+a+'">'+h.def.l+"</option>"});k+="</select>";d=jQuery(k);d.change(q);jQuery(e).append(d);jQuery(e).append('<div class="lgnd-entry lgnd-sh"><input type="checkbox" checked="checked" class="lgnd-entry-check"/>'+dlText.sha+'</div><div class="lgnd-group"></div>');n.append(e);c!=prspdata.t.length-1&&n.append("<hr/>");h=0<h.length?h[0]:null;y.push(h);h&&t(c,h)}});f.find("div.lgnd-container").show()}else f.find(".hslgnd").button("disable"),f.find("div.lgnd-container").hide();
m=jQuery("#selector-v"+a);l&V_FLAG_SEL?(m.removeAttr("disabled"),m.prop("checked",!0)):(m.attr("disabled",!0),m.prop("checked",!1));l&V_FLAG_OPT?f.find(".vopts").button("enable"):f.find(".vopts").button("disable");(m=w.hint())||"string"==typeof e.n&&""!=e.n?(f.find(".vnote").button("enable"),m=m?"string"==typeof e.n&&""!=e.n?m+(".<br/>"+e.n):m+".":e.n,jQuery("#vnotes-txt").empty().append(m)):f.find(".vnote").button("disable");w.setup();g(!1);A&&d&&w.render(A);k=w}var p={},u=0,k=null,y=[],A=null;p.getFrameID=
c;p.getIndex=function(){return a};p.setViz=function(a,h){a!=u&&(jQuery(c()+" div.view-control-bar select.view-viz-select").val(a),w(a,h))};p.initDOM=function(q){var t=document.getElementById("dltext-viewframe-dom").innerHTML;jQuery("#viz-display-frame").append('<div id="view-frame-'+a+'">'+t+"</div>");var t=jQuery(c()),k=prspdata.bClrs.vf;k&&0<k.length&&t.find("div.view-control-bar").css("background-color",k);t.find("div.lgnd-container").draggable({handle:t.find("div.lgnd-handle"),containment:"parent"});
var p=t.find("div.view-control-bar select.view-viz-select");prspdata.e.vf.forEach(function(a,c){p.append('<option value="'+c+'">'+a.l+"</option>")});p.val(q);p.change(e);t.find("div.view-control-bar button:first").button({icons:{primary:"ui-icon-bookmark"},text:!1}).click(d).next().button({icons:{primary:"ui-icon-search"},text:!1}).click(f).next().button({icons:{primary:"ui-icon-cancel"},text:!1}).click(l).next().button({icons:{primary:"ui-icon-gear"},text:!1}).click(n).next().button({icons:{primary:"ui-icon-help"},
text:!1}).click(m).next();t.find("div.lgnd-container").click(h);w(q,!1)};p.getSelLocAtts=function(a){var h=[];jQuery(c()+' div.lgnd-container div.lgnd-scroll div.lgnd-template[data-index="'+a+'"] div.lgnd-locate input:checked').each(function(){var a=jQuery(this).parent().data("id");h.push(a)});return h};p.getSelFeatAtts=function(a){var h=[],d,e;jQuery(c()+' div.lgnd-container div.lgnd-scroll div.lgnd-template[data-index="'+a+'"] div.lgnd-group div.lgnd-value input:checked').each(function(){d=jQuery(this).parent().data("index");
"number"==typeof d?h.push(d):-1!=(e=d.indexOf(","))?h.push([parseInt(d.substring(0,e),10),parseInt(d.substring(e+1),10)]):h.push(parseInt(d,10))});return h};p.getSelLegend=function(a){return y[a]};p.getLgndSels=function(){return y.slice(0)};p.setLgndSels=function(a){a.forEach(function(a,h){jQuery(c()+' div.lgnd-container div.lgnd-scroll div.lgnd-template[data-index="'+h+'"] select.lgnd-select').val(a);t(h,a)})};p.getState=function(){return k?k.getState():null};p.setState=function(a){k&&k.setState(a)};
p.showStream=function(a){A=a;k&&k.render(a)};p.setStream=function(a){A=a};p.selBtns=function(a){g(a)};p.clearSel=function(){k&&k.clearSel();g(!1)};p.setSel=function(a){return k&&k.flags()&V_FLAG_SEL?(k.setSel(a),g(0<a.length),!0):!1};p.resize=function(){k&&k.resize()};p.title=function(){return PData.getVizIndex(u).l};p.flushLgnd=function(){jQuery(c()).find("div.lgnd-container").css("left","10px")};return p}
var PData=function(){function a(a,d,e){jQuery.ajax({type:"POST",url:prspdata.ajax_url,data:{action:"prsp_get_records",tmplt_id:prspdata.t[a].id,from:d,count:e},success:function(d,q,f){q=n[a];d=JSON.parse(d);q.d=q.d?q.d.concat(d):d;q.n+=e;c()},error:function(a,c,h){alert(h)}})}function c(){for(var c=!0,d=0;d<prspdata.t.length;d++){var t=n[d].n,f=prspdata.t[d].n;if(t<f){c=!1;f-=t;a(d,t,f<e?f:e);break}}c&&(r=!0,setTimeout(function(){jQuery("body").trigger("prospect",{pstate:PSTATE_PROCESS,component:0})},
500))}var e=1E3,d,f,g,l,n=[],m=0,r=!1;return{init:function(){d=document.getElementById("dltext-from").innerHTML;f=document.getElementById("dltext-to").innerHTML;g=document.getElementById("dltext-approximately").innerHTML;l=document.getElementById("dltext-now").innerHTML;"number"==typeof prspdata.chunk&&0<prspdata.chunk&&(e=prspdata.chunk);prspdata.t.forEach(function(a){n.push({i:m,n:0,d:null});m+=a.n});c()},intersect:function(a,c){for(var d=[],e=a.length,p=c.length,f,k,g=0,l=0;g<e&&l<p;)f=a[g],k=
...
ps:JSON.parse(f)})}}c.forEach(function(c,d){c.ps.forEach(function(e){a.append('<li data-type="x" data-xid="'+c.id+'" data-xindex="'+d+'" data-id="'+e.id+'"><i class="label">'+e.l+'</i> <button class="del">'+dlText.del+'</button> <button class="edit">'+dlText.edit+"</button></li>")})})})();jQuery("#prspctv-mlist").click(function(a){if("BUTTON"==a.target.nodeName){var e=jQuery(a.target).hasClass("del"),f=jQuery(a.target).parent(),h=f.data("type"),g=f.data("id");if(e){switch(h){case "l":a=C.findIndex(function(a){return g==
a.id});-1!=a&&(C.splice(a,1),0==C.length?B.removeItem(prspdata.e.id):B.setItem(prspdata.e.id,JSON.stringify(C)));break;case "x":a=f.data("xindex"),e=c[a],a=e.ps.findIndex(function(a){return g==a.id}),-1!=a&&(e.ps.splice(a,1),d=!0)}f.remove()}else{var k;switch(h){case "l":k=_.find(C,function(a){return g==a.id});break;case "x":a=f.data("xindex"),e=c[a],k=_.find(e.ps,function(a){return g==a.id})}jQuery("#edit-psrctv-lbl").val(k.l);jQuery("#edit-psrctv-note").val(k.n);var l=jQuery("#dialog-edit-prsrctv").dialog({width:340,
height:270,modal:!0,buttons:[{text:dlText.ok,click:function(){k.l=jQuery("#edit-psrctv-lbl").val();k.n=jQuery("#edit-psrctv-note").val();f.find(".label").text(k.l);"x"==h?d=!0:B.setItem(prspdata.e.id,JSON.stringify(C));l.dialog("close")}},{text:dlText.cancel,click:function(){l.dialog("close")}}]})}}});a=jQuery("#dialog-manage-prsrctv").dialog({width:450,height:350,modal:!0,buttons:[{text:dlText.ok,click:function(){d&&c.forEach(function(a){0<a.ps.length?B.setItem(a.id,JSON.stringify(a.ps)):B.removeItem(a.id)});
...
c,d){var e;if(d)h(),e=0;else{do e=Math.floor(1E3*Math.random()+1),-1!=y.findIndex(function(a){return a.id==e})&&(e=-1);while(-1==e)}var f,g;if("_remove"==a)f=new PFilterRemove(e),g={t:[!0,!0,!0,!0]};else switch(g=PData.getAttID(a),g.def.t){case "V":f=new PFilterVocab(e,g);break;case "T":f=new PFilterText(e,g);break;case "N":f=new PFilterNum(e,g);break;case "D":f=new PFilterDates(e,g)}if(d)A=f,z=a,d=_.template(document.getElementById("dltext-selector-head").innerHTML),a=jQuery('div.filter-instance[data-id="0"]'),
...
a.s.a0);jQuery("#selector-v1").prop("checked",a.s.a1);PData.ready()&&v&&(c(),A&&t());return!0}function p(a,c){var d=prspdata.bClrs[a];d&&0<d.length&&jQuery(c).css("background-color",d)}var u,k,y=[],A=null,z,D,v,x,B=null,C=[];jQuery("body").addClass("waiting");p("cb","#command-bar");p("fs","#filter-frame");p("sf","#selector-frame");PState.init();PMapHub.init(prspdata.m);(function(){function a(d,e){c=document.getElementById(d).innerHTML;dlText[e]=c.trim()}var c;a("dltext-removehideall","rha");a("dltext-showhideall",
...
text:!1}).click(function(a){jQuery("#selector-instance").slideToggle(400);a.preventDefault()});jQuery("#btn-apply-selector").button({icons:{primary:"ui-icon-arrowreturn-1-e"},text:!1}).click(function(a){t();PState.set(PSTATE_READY);a.preventDefault()});jQuery("#dialog-about .logo").attr("src",prspdata.assets+"prospectlogo.jpg");jQuery("#btn-inspect-left").button({icons:{primary:"ui-icon-arrowthick-1-w"},text:!1});jQuery("#btn-inspect-right").button({icons:{primary:"ui-icon-arrowthick-1-e"},text:!1});
(function(){jQuery("#filter-list").append('<li class="remove" data-id="_remove"><i>'+dlText.rha+"</i></li>");prspdata.a.forEach(function(a){switch(a.def.t){case "V":case "T":case "N":case "D":jQuery("#filter-list").append('<li data-id="'+a.id+'">'+a.def.l+"</li>")}})})();0!=prspdata.show_prspctv.length&&w(prspdata.show_prspctv)||(u=PViewFrame(0),u.initDOM(0),e(""));jQuery(window).resize(function(){u&&u.resize();k&&k.resize()});jQuery("body").on("prospect",function(a,d){d.pstate==PSTATE_PROCESS&&(PState.set(PSTATE_PROCESS),
c(),A&&t(),PState.set(PSTATE_READY),jQuery("body").removeClass("waiting"))});PState.set(PSTATE_LOAD);PData.init()});function onYouTubeIframeAPIReady(){widgetData.ytCall&&widgetData.ytCall()};