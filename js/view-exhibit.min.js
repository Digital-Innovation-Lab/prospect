...
a.x?"10":"-10"}).attr("transform",function(a){return 180>a.x?"":"rotate(180)"}).style("text-anchor",function(a){return 180>a.x?"start":"end"}).attr("fill","black").attr("stroke","none").text(function(a){return a.r.l}).on("click",function(a){PState.set(PSTATE_UPDATE);h.each(function(a){a.linked=!1});f.each(function(d){if(d.s===a){d.t.linked=!0;for(var k=0;k<e.length;k++){var f=e[k];if(f.source===a&&f.target===d.t){d3.select(this).attr("stroke",f.c).classed("thick",!0);this.parentElement.appendChild(this);
break}}}else if(d.t===a)for(d.s.linked=!0,k=0;k<e.length;k++){if(f=e[k],f.target===a&&f.source===d.s){d3.select(this).attr("stroke",f.c).classed("thick",!0);this.parentElement.appendChild(this);break}}else d3.select(this).attr("stroke","black").classed("thick",!1)});h.select("text").attr("fill",function(a){return a.linked?"white":"black"}).attr("stroke","none");d3.select(this).attr("fill","red").attr("stroke","black");PState.set(PSTATE_READY)});g=d3.layout.bundle();m=d3.svg.line.radial().interpolate("bundle").tension(.85).radius(function(a){return a.y}).angle(function(a){return a.x/
