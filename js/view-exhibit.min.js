...
E(c,b))})};f.getState=function(){return h?h.getState():null};f.setState=function(b){h&&h.setState(b)};f.showStream=function(b){u=b;h&&h.render(b);t(!1)};f.setStream=function(b){u=b};f.selBtns=function(b){B(b)};f.clearSel=function(){h&&h.clearSel();B(!1)};f.setSel=function(b){return h&&h.flags()&V_FLAG_SEL?(h.setSel(b),B(0<b.length),!0):!1};f.vizAddSel=function(b){};f.vizDelSel=function(b){};f.resize=function(){h&&h.resize()};f.title=function(){return PData.vByN(H).l};f.flushLgnd=function(){jQuery(n()).find("div.lgnd-container").css("left",
"10px")};f.getBMData=function(){return h?{t:h.tUsed,r:h.rMap}:null};return f}
